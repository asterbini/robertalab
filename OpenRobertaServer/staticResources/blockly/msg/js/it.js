// This file was automatically generated.  Do not modify.

'use strict';

goog.provide('Blockly.Msg.it');

goog.require('Blockly.Msg');

Blockly.Msg.ACCELERATION = "acceleration mg";
Blockly.Msg.ACCELERATION_TOOLTIP = "Get the acceleration value in milli-gravitys.";
Blockly.Msg.ACCELEROMETER_ROTATION = "rotation °";
Blockly.Msg.ACCELEROMETER_ROTATION_TOOLTIP = "Get the tilt or rotations in degrees.";
Blockly.Msg.ACCELEROMETER_TOOLTIP = "Represents an accelerometer.";
Blockly.Msg.ACTIVITY_TOOLTIP = "Marcatore per attivita addizionale.";
Blockly.Msg.ACTOR_TOOLTIP = "Rappresenta un attore.";
Blockly.Msg.ADD_COMMENT = "Aggiungi commento";
Blockly.Msg.AMBIENTLIGHT_TOOLTIP = "Represents an ambientlight sensor.";
Blockly.Msg.ANALOG = "analog";
Blockly.Msg.ARDUBRICK_TOOLTIP = "Represents the Bot'n Roll board with connected actors and sensors. There are also inbuilt actors and sensors available, e.g. pushbuttons, display ...";
Blockly.Msg.AUTH = "Autorizza questa applicazione a salvare il tuo lavoro per essere condiviso.";
Blockly.Msg.BATTERY_GETSAMPLE_TOOLTIP = "Gets the current voltage from the battery.";
Blockly.Msg.BOB3_ARM = "arm";
Blockly.Msg.BOB3_ARM_PAIR = "pad pair";
Blockly.Msg.BOTH = "entrambi";
Blockly.Msg.BRICKLIGHT = "luce del brick EV3";
Blockly.Msg.BRICKLIGHT_BLUE = "blu";
Blockly.Msg.BRICKLIGHT_COLOR = "colore";
Blockly.Msg.BRICKLIGHT_DOUBLE_FLASH = "doppio lampeggio";
Blockly.Msg.BRICKLIGHT_FLASH = "singolo lampeggio";
Blockly.Msg.BRICKLIGHT_GREEN = "verde";
Blockly.Msg.BRICKLIGHT_OFF_TOOLTIP = "Spegne la luce del brick EV3.";
Blockly.Msg.BRICKLIGHT_ON = "sempre acceso";
Blockly.Msg.BRICKLIGHT_ON_TOOLTIP = "Accende la luce del brick EV3.";
Blockly.Msg.BRICKLIGHT_ORANGE = "arancione";
Blockly.Msg.BRICKLIGHT_RED = "rosso";
Blockly.Msg.BRICKLIGHT_RESET_TOOLTIP = "Riporta la luce del brick EV3 al valore predefinito: verde e lampeggiante.";
Blockly.Msg.BRICK_TRACK_WIDTH = "interasse (cm)";
Blockly.Msg.BRICK_WHEEL_DIAMETER = "diametro della ruota (cm)";
Blockly.Msg.BUTTON_DO_SHARE = "Condividi";
Blockly.Msg.BUTTON_EMPTY_LIST = "Lista vuota";
Blockly.Msg.CALLIOPEBRICK_TOOLTIP = "Represents Calliope, a pocket-sized codeable computer. There are also inbuilt actors and sensors available, e.g. buttons, display ...";
Blockly.Msg.CENTER = "centra";
Blockly.Msg.CHANGE_VALUE_TITLE = "Modifica valore:";
Blockly.Msg.CHAT = "Chatta con il tuo amico scrivendo in questo box!";
Blockly.Msg.CLEAN_UP = "Pulisci i blocchi";
Blockly.Msg.COLLAPSE_ALL = "Comprimi blocchi";
Blockly.Msg.COLLAPSE_BLOCK = "Comprimi blocco";
Blockly.Msg.COLOUR_BLEND_COLOUR1 = "colore 1";
Blockly.Msg.COLOUR_BLEND_COLOUR2 = "colore 2";
Blockly.Msg.COLOUR_BLEND_HELPURL = "http://meyerweb.com/eric/tools/color-blend/";
Blockly.Msg.COLOUR_BLEND_RATIO = "rapporto";
Blockly.Msg.COLOUR_BLEND_TITLE = "miscela";
Blockly.Msg.COLOUR_BLEND_TOOLTIP = "Mescola due colori con un determinato rapporto (0.0 - 1.0).";
Blockly.Msg.COLOUR_GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore di colore/luce.";
Blockly.Msg.COLOUR_PICKER_HELPURL = "https://it.wikipedia.org/wiki/Colore";
Blockly.Msg.COLOUR_PICKER_TOOLTIP = "Scegli un colore dalla tavolozza.";
Blockly.Msg.COLOUR_RANDOM_HELPURL = "http://randomcolour.com";
Blockly.Msg.COLOUR_RANDOM_TITLE = "colore casuale";
Blockly.Msg.COLOUR_RANDOM_TOOLTIP = "Scegli un colore a caso.";
Blockly.Msg.COLOUR_RGB_BLUE = "blu";
Blockly.Msg.COLOUR_RGB_GREEN = "verde";
Blockly.Msg.COLOUR_RGB_HELPURL = "http://www.december.com/html/spec/colorper.html";
Blockly.Msg.COLOUR_RGB_RED = "rosso";
Blockly.Msg.COLOUR_RGB_TITLE = "colora con";
Blockly.Msg.COLOUR_RGB_TOOLTIP = "Crea un colore con la quantità specificata di rosso, verde e blu. Tutti i valori devono essere compresi tra 0 e 100.";
Blockly.Msg.COLOUR_RGB_WHITE = "bianco";
Blockly.Msg.COLOUR_TOOLTIP = "Rappresenta un sensore di colore/luce.";
Blockly.Msg.COMPASS_GETSAMPLE_TOOLTIP = "Gets the current reading from the compass sensor.";
Blockly.Msg.COMPASS_TOOLTIP = "Represents a compass sensor.";
Blockly.Msg.CONFIGURATION_ERROR_MOTORS_ROTATION_DIRECTION = "The direction of rotation of the left and right motor is different!";
Blockly.Msg.CONFIGURATION_ERROR_MOTOR_LEFT_MISSING = "Il motore sinistro manca nella configurazione!";
Blockly.Msg.CONFIGURATION_ERROR_MOTOR_LEFT_UNREGULATED = "Il motore sinistro non è regolato!";
Blockly.Msg.CONFIGURATION_ERROR_MOTOR_MISSING = "Manca il motore collegato a questa porta!!";
Blockly.Msg.CONFIGURATION_ERROR_MOTOR_RIGHT_MISSING = "Il motore destro manca nella configurazione!";
Blockly.Msg.CONFIGURATION_ERROR_MOTOR_RIGHT_UNREGULATED = "Il motore deistro non è regolato!";
Blockly.Msg.CONFIGURATION_ERROR_MULTIPLE_LEFT_MOTORS = "You have multiple left motors assigned to your configuration!";
Blockly.Msg.CONFIGURATION_ERROR_MULTIPLE_RIGHT_MOTORS = "You have multiple right motors assigned to your configuration!";
Blockly.Msg.CONFIGURATION_ERROR_SENSOR_MISSING = "This sensor is not set to the port!";
Blockly.Msg.CONFIGURATION_ERROR_SENSOR_WRONG = "Il sensore collegato alla porta è sbagliato!";
Blockly.Msg.CONFLIST_DELETE_ALL_TOOLTIP = "Click here to delete all selected programs.";
Blockly.Msg.CONFLIST_DELETE_TOOLTIP = "Click here to delete your robot configuration.";
Blockly.Msg.CONFLIST_LOAD_TOOLTIP = "Click here to load your robot configuration in the configuration environment.";
Blockly.Msg.CONNECTION_CHECK = "connection to robot %1 active?";
Blockly.Msg.CONNECTION_CHECK_TOOLTIP = "Check if the connection to the robot is active.";
Blockly.Msg.CONNECTION_CONNECT = "connetti con il robot di nome ";
Blockly.Msg.CONNECTION_FROM_CONNECTION = "from connection";
Blockly.Msg.CONNECTION_FROM_ROBOT = "dal robot";
Blockly.Msg.CONNECTION_MBED_RECEIVE_TOOLTIP = "Reads a message over one of the channels (0 - 255). The default channel is 0.";
Blockly.Msg.CONNECTION_MBED_SEND_TOOLTIP = "Sends a message to another microBoard. You can specify a signal strength from 0 - 7, where 0 is very low and 7 is the strongests. The message is send over channel 0 unless you specify another one.";
Blockly.Msg.CONNECTION_OVER_CHANNEL = "over channel";
Blockly.Msg.CONNECTION_POWER = "with strength";
Blockly.Msg.CONNECTION_PROTOCOL_BLUETOOTH = "Bluetooth";
Blockly.Msg.CONNECTION_RECEIVED_DATA = "messaggio ricevuto (dati ricevuti)";
Blockly.Msg.CONNECTION_RECEIVE_TOOLTIP = "Aspetta un messaggio dal robot indicato nella connessione.";
Blockly.Msg.CONNECTION_SEND_DATA = "manda messaggio (dati da mandare)";
Blockly.Msg.CONNECTION_SEND_TOOLTIP = "Invia un messaggio ad altro robot.";
Blockly.Msg.CONNECTION_SET_CHANNEL = "set channel to %1";
Blockly.Msg.CONNECTION_SET_CHANNEL_TOOLTIP = "Sets the channel for sending and receiving messages. Can be set from 0 to 255.";
Blockly.Msg.CONNECTION_START_TOOLTIP = "Cerca di collegarsi con altro robot tramite Bluetooth.";
Blockly.Msg.CONNECTION_TOOLTIP = "A robot's connection";
Blockly.Msg.CONNECTION_TO_CONNECTION = "to connection";
Blockly.Msg.CONNECTION_TO_ROBOT = "al robot";
Blockly.Msg.CONNECTION_WAIT_FOR_CONNECTION = "aspetta la connessione";
Blockly.Msg.CONNECTION_WAIT_TOOLTIP = "Aspetta una connessione Bluetooth.";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_HELPURL = "https://github.com/google/blockly/wiki/Loops#loop-termination-blocks";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_OPERATOR_BREAK = "esce dal ciclo";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_OPERATOR_CONTINUE = "prosegui con la successiva ripetizione del ciclo";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_TOOLTIP_BREAK = "Esce dal ciclo.";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_TOOLTIP_CONTINUE = "Salta il resto di questo ciclo e prosegue con la successiva ripetizione.";
Blockly.Msg.CONTROLS_FLOW_STATEMENTS_WARNING = "Attenzioneː Questo blocco può essere usato solo in un ciclo.";
Blockly.Msg.CONTROLS_FOREACH_HELPURL = "https://github.com/google/blockly/wiki/Loops#for-each";
Blockly.Msg.CONTROLS_FOREACH_TITLE = "per ciascun %1 nella lista %2";
Blockly.Msg.CONTROLS_FOREACH_TOOLTIP = "Per ogni elemento in una lista, imposta la variabile '%1' a quell'elemento e quindi esegue le istruzioni.";
Blockly.Msg.CONTROLS_FOR_HELPURL = "https://github.com/google/blockly/wiki/Loops#count-with";
Blockly.Msg.CONTROLS_FOR_TITLE = "ripeti con la variabile %1 che va da %2 a %3 a passi di %4";
Blockly.Msg.CONTROLS_FOR_TOOLTIP = "Fa sì che la variabile '%1' prenda tutti i valori a partire dal numero di partenza fino a quello di arrivo, con passo pari all'intervallo specificato, ed esegue il blocco indicato.";
Blockly.Msg.CONTROLS_IF_ELSEIF_TOOLTIP = "Aggiungi una condizione al blocco se.";
Blockly.Msg.CONTROLS_IF_ELSE_TOOLTIP = "Aggiungi una condizione finale pigliatutto al blocco se.";
Blockly.Msg.CONTROLS_IF_HELPURL = "https://github.com/google/blockly/wiki/IfElse";
Blockly.Msg.CONTROLS_IF_IF_TOOLTIP = "Aggiungi, elimina o riordina le sezioni per riconfigurare questo blocco se.";
Blockly.Msg.CONTROLS_IF_MSG_ELSE = "altrimenti";
Blockly.Msg.CONTROLS_IF_MSG_ELSEIF = "altrimenti se";
Blockly.Msg.CONTROLS_IF_MSG_IF = "se";
Blockly.Msg.CONTROLS_IF_TOOLTIP_1 = "Se un valore è vero allora esegue le istruzioni.";
Blockly.Msg.CONTROLS_IF_TOOLTIP_2 = "Se un valore è vero allora esegue il primo blocco di istruzioni. Altrimenti esegue il secondo blocco di istruzioni.";
Blockly.Msg.CONTROLS_IF_TOOLTIP_3 = "Se il primo valore è vero allora esegue un primo blocco di istruzioni. Altrimenti, se il secondo valore è vero, esegue un secondo blocco di istruzioni.";
Blockly.Msg.CONTROLS_IF_TOOLTIP_4 = "Se il primo valore è vero allora esegue un primo blocco di istruzioni. Altrimenti, se il secondo valore è vero, esegue un secondo blocco di istruzioni. Se nessuno dei valori è vero esegue l'ultimo blocco di istruzioni.";
Blockly.Msg.CONTROLS_REPEAT_HELPURL = "https://it.wikipedia.org/wiki/Ciclo_for";
Blockly.Msg.CONTROLS_REPEAT_INPUT_DO = "fai";
Blockly.Msg.CONTROLS_REPEAT_TITLE = "ripeti %1 volte";
Blockly.Msg.CONTROLS_REPEAT_TOOLTIP = "Esegue le istruzioni più volte.";
Blockly.Msg.CONTROLS_WHILEUNTIL_HELPURL = "https://github.com/google/blockly/wiki/Loops#repeat";
Blockly.Msg.CONTROLS_WHILEUNTIL_OPERATOR_UNTIL = "ripeti se falso";
Blockly.Msg.CONTROLS_WHILEUNTIL_OPERATOR_WHILE = "ripeti se vero";
Blockly.Msg.CONTROLS_WHILEUNTIL_TOOLTIP_UNTIL = "Finché un valore è falso, esegue le istruzioni.";
Blockly.Msg.CONTROLS_WHILEUNTIL_TOOLTIP_WHILE = "Finché un valore è vero, esegue le istruzioni.";
Blockly.Msg.DATATABLE_ACTUALIZATION = "Data di salvataggio";
Blockly.Msg.DATATABLE_BY = "by";
Blockly.Msg.DATATABLE_CONFIGURATIONS = "configurazioni";
Blockly.Msg.DATATABLE_CONFIGURATION_NAME = "Nome della configurazione";
Blockly.Msg.DATATABLE_CREATED_BY = "Creatore";
Blockly.Msg.DATATABLE_CREATED_ON = "Data di creazione";
Blockly.Msg.DATATABLE_DATE = "modified";
Blockly.Msg.DATATABLE_NUMBER_OF_BLOCKS = "blocks";
Blockly.Msg.DATATABLE_PROGRAMS = "programmi";
Blockly.Msg.DATATABLE_PROGRAM_NAME = "Nome del programma";
Blockly.Msg.DATATABLE_SHARED = "Condiviso";
Blockly.Msg.DATATABLE_SHARED_WITH = "Condiviso con";
Blockly.Msg.DELETE_ALL_BLOCKS = "Cancellare tutti i %1 blocchi?";
Blockly.Msg.DELETE_BLOCK = "Cancella blocco";
Blockly.Msg.DELETE_X_BLOCKS = "Cancella %1 blocchi";
Blockly.Msg.DIGITAL = "digital";
Blockly.Msg.DISABLE_BLOCK = "Disattiva blocco";
Blockly.Msg.DISPLAY_ANIMATION = "animation";
Blockly.Msg.DISPLAY_CHARACTER = "character";
Blockly.Msg.DISPLAY_CLEAR = "cancella lo schermo";
Blockly.Msg.DISPLAY_CLEAR_TOOLTIP = "Cancella lo schermo.";
Blockly.Msg.DISPLAY_COL = "alla colonna";
Blockly.Msg.DISPLAY_GET_BRIGHTNESS_TOOLTIP = "Returns the brightness for all leds of the display. 0 means all leds are turned off, 9 is the brightest value.";
Blockly.Msg.DISPLAY_GET_PIXEL_TOOLTIP = "Returns the brightness for this led. 0 means the led is turned off, 9 is the brightest value.";
Blockly.Msg.DISPLAY_IMAGE = "image";
Blockly.Msg.DISPLAY_PICTURE = "l'immagine";
Blockly.Msg.DISPLAY_PICTURE_EYES_CLOSED = "occhi chiusi";
Blockly.Msg.DISPLAY_PICTURE_EYES_OPEN = "occhi aperti";
Blockly.Msg.DISPLAY_PICTURE_FLOWERS = "fiori";
Blockly.Msg.DISPLAY_PICTURE_GLASSES = "occhiali";
Blockly.Msg.DISPLAY_PICTURE_TACHO = "tachimetro";
Blockly.Msg.DISPLAY_PICTURE_TOOLTIP = "Mostra una immagine sullo schermo mettendo l'angolo superiore sinistro alle coordinate indicate (in pixel).";
Blockly.Msg.DISPLAY_PIXEL_BRIGHTNESS = "luminosità";
Blockly.Msg.DISPLAY_PIXEL_TITLE = "LED";
Blockly.Msg.DISPLAY_ROW = "alla riga";
Blockly.Msg.DISPLAY_SET_BRIGHTNESS_TOOLTIP = "Sets the brightness for all leds of the display. 0 means all leds are turned off, 9 is the brightest value.";
Blockly.Msg.DISPLAY_SET_PIXEL_TOOLTIP = "Sets the brightness for this led. 0 means the led is turned off, 9 is the brightest value.";
Blockly.Msg.DISPLAY_SHOW = "mostra";
Blockly.Msg.DISPLAY_TEXT = "il testo";
Blockly.Msg.DISPLAY_TEXT_TOOLTIP = "Mostra un testo sullo schermo alla riga e colonna indicata (in caratteri).";
Blockly.Msg.DUPLICATE_BLOCK = "Duplica";
Blockly.Msg.ENABLE_BLOCK = "Attiva blocco";
Blockly.Msg.ENCODER_GETSAMPLE_TOOLTIP = "Ottiene la lettura corrente del encoder motore.";
Blockly.Msg.ENCODER_RESET_TOOLTIP = "Azzera l'encoder del motore.";
Blockly.Msg.EV3BRICK_TOOLTIP = "Mattone EV3 con attuatori e sensori collegati. Sono disponibili anche attuatori e sensori interni al brick, per es. bottoni, schermo, altopralante ...";
Blockly.Msg.EXPAND_ALL = "Espandi blocchi";
Blockly.Msg.EXPAND_BLOCK = "Espandi blocco";
Blockly.Msg.EXTERNAL_INPUTS = "Ingressi esterni";
Blockly.Msg.FLAME_GETSAMPLE_TOOLTIP = "Gets the current reading from the flame sensor.";
Blockly.Msg.FLAME_TOOLTIP = "Represents a flame sensor.";
Blockly.Msg.FOR = "per";
Blockly.Msg.GEARED_MOTOR = "Geared motor";
Blockly.Msg.GET = "ottieni";
Blockly.Msg.GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore scelto.";
Blockly.Msg.GET_CODE_TOOLTIP = "Returns the value of the solderable code pad in the head piece. Values are in range 0-31.";
Blockly.Msg.GYRO_GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore giroscopio.";
Blockly.Msg.GYRO_RESET_TOOLTIP = "Azzera il sensore giroscopio.";
Blockly.Msg.GYRO_TOOLTIP = "Rappresenta un sensore giroscopio.";
Blockly.Msg.HELP = "Aiuto";
Blockly.Msg.IF_TOOLTIP = "Controlla la condizione in 'se'. Se la condizione è vera, esegue l'azione 'da fare'.";
Blockly.Msg.IMAGE_GET_TOOLTIP = "Returns the chosen image.";
Blockly.Msg.IMAGE_GET_TOOLTIP_ANGRY = "angry";
Blockly.Msg.IMAGE_GET_TOOLTIP_ASLEEP = "asleep";
Blockly.Msg.IMAGE_GET_TOOLTIP_BUTTERFLY = "butterfly";
Blockly.Msg.IMAGE_GET_TOOLTIP_CHESSBOARD = "chessboard";
Blockly.Msg.IMAGE_GET_TOOLTIP_CONFUSED = "confused";
Blockly.Msg.IMAGE_GET_TOOLTIP_COW = "cow";
Blockly.Msg.IMAGE_GET_TOOLTIP_DIAMOND = "diamond";
Blockly.Msg.IMAGE_GET_TOOLTIP_DIAMOND_SMALL = "small diamond";
Blockly.Msg.IMAGE_GET_TOOLTIP_DUCK = "duck";
Blockly.Msg.IMAGE_GET_TOOLTIP_FABULOUS = "fabulous";
Blockly.Msg.IMAGE_GET_TOOLTIP_GHOST = "ghost";
Blockly.Msg.IMAGE_GET_TOOLTIP_GIRAFFE = "giraffe";
Blockly.Msg.IMAGE_GET_TOOLTIP_HEART = "heart";
Blockly.Msg.IMAGE_GET_TOOLTIP_HEART_SMALL = "small heart";
Blockly.Msg.IMAGE_GET_TOOLTIP_HOUSE = "house";
Blockly.Msg.IMAGE_GET_TOOLTIP_MEH = "meh!";
Blockly.Msg.IMAGE_GET_TOOLTIP_MUSIC_CROTCHET = "music crotchet";
Blockly.Msg.IMAGE_GET_TOOLTIP_MUSIC_QUAVER = "music quaver";
Blockly.Msg.IMAGE_GET_TOOLTIP_MUSIC_QUAVERS = "music quavers";
Blockly.Msg.IMAGE_GET_TOOLTIP_NO = "no";
Blockly.Msg.IMAGE_GET_TOOLTIP_PACMAN = "pacman";
Blockly.Msg.IMAGE_GET_TOOLTIP_PITCHFORK = "pitchfork";
Blockly.Msg.IMAGE_GET_TOOLTIP_RABBIT = "rabbit";
Blockly.Msg.IMAGE_GET_TOOLTIP_ROLLERSKATE = "rollerskate";
Blockly.Msg.IMAGE_GET_TOOLTIP_SAD = "sad";
Blockly.Msg.IMAGE_GET_TOOLTIP_SILLY = "silly";
Blockly.Msg.IMAGE_GET_TOOLTIP_SKULL = "skull";
Blockly.Msg.IMAGE_GET_TOOLTIP_SMILE = "smile";
Blockly.Msg.IMAGE_GET_TOOLTIP_SNAKE = "snake";
Blockly.Msg.IMAGE_GET_TOOLTIP_SQUARE = "square";
Blockly.Msg.IMAGE_GET_TOOLTIP_SQUARE_SMALL = "small square";
Blockly.Msg.IMAGE_GET_TOOLTIP_STICKFIGURE = "stickfigure";
Blockly.Msg.IMAGE_GET_TOOLTIP_SWORD = "sword";
Blockly.Msg.IMAGE_GET_TOOLTIP_TARGET = "target";
Blockly.Msg.IMAGE_GET_TOOLTIP_TORTOISE = "tortoise";
Blockly.Msg.IMAGE_GET_TOOLTIP_TRIANGLE = "triangle";
Blockly.Msg.IMAGE_GET_TOOLTIP_TRIANGLE_LEFT = "triangle left";
Blockly.Msg.IMAGE_GET_TOOLTIP_TSHIRT = "T-shirt";
Blockly.Msg.IMAGE_GET_TOOLTIP_UMBRELLA = "umbrella";
Blockly.Msg.IMAGE_GET_TOOLTIP_XMAS = "xmas";
Blockly.Msg.IMAGE_GET_TOOLTIP_YES = "yes";
Blockly.Msg.IMAGE_INVERT = "invert";
Blockly.Msg.IMAGE_INVERT_TOOLTIP = "Inverts the image. Each pixel with value 0 or none will be set to # or 9 and pixels with value # or 9 will be set to 0 or none.";
Blockly.Msg.IMAGE_SHIFT = "shift";
Blockly.Msg.IMAGE_SHIFT_TOOLTIP = "Shifts the image in the given direction by the given number";
Blockly.Msg.IMAGE_TOOLTIP = "Creates an image for the display.";
Blockly.Msg.INFRARED_GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore infrarosso.";
Blockly.Msg.INFRARED_TOOLTIP = "Rappresenta un sensore infrarosso.";
Blockly.Msg.INLINE_INPUTS = "Ingressi in linea";
Blockly.Msg.INTERNAL_PORT = "internal";
Blockly.Msg.JOYSTICK_GETSAMPLE_TOOLTIP = "Gets the current reading of one of the axises of the joystick";
Blockly.Msg.JOYSTICK_TOOLTIP = "Represents a joystick.";
Blockly.Msg.KEY_ISPRESSED_TOOLTIP = "Il pulstante indicato è premuto?";
Blockly.Msg.LED = "LED";
Blockly.Msg.LED_MATRIX = "griglia di LED";
Blockly.Msg.LED_MATRIX_TOOLTIP = "Rappresenta una griglia di LED.";
Blockly.Msg.LED_OFF = "spegne il LED";
Blockly.Msg.LED_ON = "accende il LED";
Blockly.Msg.LED_ON_TOOLTIP = "Accende il LED e ne cambia il colore.";
Blockly.Msg.LED_TOOLTIP = "Rappresenta un LED esterno (collegato).";
Blockly.Msg.LIGHT_ARDU_TOOLTIP = "Rappresenta 8 sensori di luce.";
Blockly.Msg.LIGHT_GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore di luce.";
Blockly.Msg.LIGHT_TOOLTIP = "Rappresenta un sensore di luce.";
Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL = "https://github.com/google/blockly/wiki/Lists#create-empty-list";
Blockly.Msg.LISTS_CREATE_EMPTY_TITLE = "crea lista vuota di";
Blockly.Msg.LISTS_CREATE_EMPTY_TOOLTIP = "Restituisce una lista, di lunghezza 0, che non contiene nessun dato.";
Blockly.Msg.LISTS_CREATE_TITLE = "crea lista con";
Blockly.Msg.LISTS_CREATE_WITH_CONTAINER_TITLE_ADD = "lista di";
Blockly.Msg.LISTS_CREATE_WITH_CONTAINER_TOOLTIP = "Aggiungi, rimuovi o riordina le sezioni per riconfigurare il blocco lista.";
Blockly.Msg.LISTS_CREATE_WITH_HELPURL = "https://github.com/google/blockly/wiki/Lists#create-list-with";
Blockly.Msg.LISTS_CREATE_WITH_INPUT_WITH = "crea lista con";
Blockly.Msg.LISTS_CREATE_WITH_ITEM_TOOLTIP = "Aggiunge un elemento alla lista.";
Blockly.Msg.LISTS_CREATE_WITH_TOOLTIP = "Crea una lista con un certo numero di elementi.";
Blockly.Msg.LISTS_GET_INDEX_FIRST = "primo";
Blockly.Msg.LISTS_GET_INDEX_FROM_END = "alla posizione (contando dalla fine)";
Blockly.Msg.LISTS_GET_INDEX_FROM_START = "alla posizione";
Blockly.Msg.LISTS_GET_INDEX_GET = "prendi";
Blockly.Msg.LISTS_GET_INDEX_GET_REMOVE = "prendi e rimuovi";
Blockly.Msg.LISTS_GET_INDEX_LAST = "ultimo";
Blockly.Msg.LISTS_GET_INDEX_RANDOM = "in posizione casuale";
Blockly.Msg.LISTS_GET_INDEX_REMOVE = "rimuovi";
Blockly.Msg.LISTS_GET_INDEX_TAIL = "";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FIRST = "Restituisce il primo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FROM_END = "Restituisce l'elemento nella posizione indicata della lista. 1 corrisponde all'ultimo elemento.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FROM_START = "Restituisce l'elemento nella posizione indicata della lista. 1 corrisponde al primo elemento della lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_LAST = "Restituisce l'ultimo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_RANDOM = "Restituisce un elemento casuale in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FIRST = "Rimuove e restituisce il primo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FROM_END = "Rimuove e restituisce l'elemento nella posizione indicata in una lista. 1 corrisponde all'ultimo elemento.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FROM_START = "Rimuove e restituisce l'elemento nella posizione indicata in una lista. 1 corrisponde al primo elemento.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_LAST = "Restituisce e rimuove l'ultimo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_RANDOM = "Restituisce e rimuove un elemento casuale in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FIRST = "Rimuove il primo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FROM_END = "Rimuove l'elemento nella posizione indicata in una lista. 1 corrisponde all'ultimo elemento.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FROM_START = "Rimuove l'elemento nella posizione indicata in una lista. 1 corrisponde al primo elemento.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_LAST = "Rimuove l'ultimo elemento in una lista.";
Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_RANDOM = "Rimuove un elemento casuale in una lista.";
Blockly.Msg.LISTS_GET_SUBLIST_END_FROM_END = "alla posizione (contando dalla fine)";
Blockly.Msg.LISTS_GET_SUBLIST_END_FROM_START = "alla posizione";
Blockly.Msg.LISTS_GET_SUBLIST_END_LAST = "alla fine";
Blockly.Msg.LISTS_GET_SUBLIST_HELPURL = "https://github.com/google/blockly/wiki/Lists#getting-a-sublist";
Blockly.Msg.LISTS_GET_SUBLIST_START_FIRST = "prendi la sotto-lista dall'inizio";
Blockly.Msg.LISTS_GET_SUBLIST_START_FROM_END = "prendi la sotto-lista dalla posizione (contando dalla fine)";
Blockly.Msg.LISTS_GET_SUBLIST_START_FROM_START = "prendi la sotto-lista dalla posizione";
Blockly.Msg.LISTS_GET_SUBLIST_TAIL = "";
Blockly.Msg.LISTS_GET_SUBLIST_TOOLTIP = "Crea una copia della porzione specificata di una lista.";
Blockly.Msg.LISTS_INDEX_OF_FIRST = "trova la prima occorrenza dell'elemento";
Blockly.Msg.LISTS_INDEX_OF_HELPURL = "https://github.com/google/blockly/wiki/Lists#getting-items-from-a-list";
Blockly.Msg.LISTS_INDEX_OF_LAST = "trova l'ultima occorrenza dell'elemento";
Blockly.Msg.LISTS_INDEX_OF_TOOLTIP = "Restituisce l'indice della prima/ultima occorrenza dell'elemento nella lista. Restituisce 0 se l'elemento non viene trovato.";
Blockly.Msg.LISTS_INLIST = "nella lista";
Blockly.Msg.LISTS_ISEMPTY_HELPURL = "https://github.com/google/blockly/wiki/Lists#is-empty";
Blockly.Msg.LISTS_ISEMPTY_TITLE = "la lista %1 è vuota?";
Blockly.Msg.LISTS_ISEMPTY_TOOLTIP = "Restituisce vero se la lista è vuota.";
Blockly.Msg.LISTS_LENGTH_HELPURL = "https://github.com/google/blockly/wiki/Lists#length-of";
Blockly.Msg.LISTS_LENGTH_TITLE = "lunghezza della lista %1";
Blockly.Msg.LISTS_LENGTH_TOOLTIP = "Restituisce la lunghezza della lista";
Blockly.Msg.LISTS_REPEAT_HELPURL = "https://github.com/google/blockly/wiki/Lists#create-list-with";
Blockly.Msg.LISTS_REPEAT_TITLE = "crea una lista con l'elemento %1 ripetuto %2 volte";
Blockly.Msg.LISTS_REPEAT_TOOLTIP = "Crea una lista costituita dal valore indicato ripetuto per il numero di volte specificato.";
Blockly.Msg.LISTS_SET_INDEX_HELPURL = "https://github.com/google/blockly/wiki/Lists#in-list--set";
Blockly.Msg.LISTS_SET_INDEX_INPUT_TO = "col valore";
Blockly.Msg.LISTS_SET_INDEX_INSERT = "inserisci il valore";
Blockly.Msg.LISTS_SET_INDEX_SET = "sostituisci il valore";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FIRST = "Inserisci l'elemento all'inizio della lista.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FROM_END = "Inserisci l'elemento nella posizione indicata in una lista. #1 corrisponde all'ultimo elemento.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FROM_START = "Inserisci un elemento nella posizione indicata in una lista. #1 corrisponde al primo elemento.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_LAST = "Aggiungi un elemento alla fine di una lista";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_RANDOM = "Inserisce l'elemento casualmente in una lista.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FIRST = "Imposta il primo elemento in una lista.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FROM_END = "Imposta l'elemento nella posizione indicata di una lista. 1 corrisponde all'ultimo elemento.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FROM_START = "Imposta l'elemento nella posizione indicata di una lista. 1 corrisponde al primo elemento.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_LAST = "Imposta l'ultimo elemento in una lista.";
Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_RANDOM = "Imposta un elemento casuale in una lista.";
Blockly.Msg.LISTS_SORT_HELPURL = "https://github.com/google/blockly/wiki/Lists#sorting-a-list";
Blockly.Msg.LISTS_SORT_ORDER_ASCENDING = "crescente";
Blockly.Msg.LISTS_SORT_ORDER_DESCENDING = "decrescente";
Blockly.Msg.LISTS_SORT_TITLE = "ordinamento %1 %2 %3";
Blockly.Msg.LISTS_SORT_TOOLTIP = "Ordina una copia di un elenco.";
Blockly.Msg.LISTS_SORT_TYPE_IGNORECASE = "alfabetico, ignorare differenze maiuscole e minuscole";
Blockly.Msg.LISTS_SORT_TYPE_NUMERIC = "numerico";
Blockly.Msg.LISTS_SORT_TYPE_TEXT = "alfabetico";
Blockly.Msg.LISTS_SPLIT_HELPURL = "https://github.com/google/blockly/wiki/Lists#splitting-strings-and-joining-lists";
Blockly.Msg.LISTS_SPLIT_LIST_FROM_TEXT = "crea lista da testo";
Blockly.Msg.LISTS_SPLIT_TEXT_FROM_LIST = "crea testo da lista";
Blockly.Msg.LISTS_SPLIT_TOOLTIP_JOIN = "Unisci una lista di testi in un unico testo, separato da un delimitatore.";
Blockly.Msg.LISTS_SPLIT_TOOLTIP_SPLIT = "Dividi il testo in un elenco di testi, interrompendo ad ogni delimitatore.";
Blockly.Msg.LISTS_SPLIT_WITH_DELIMITER = "con delimitatore";
Blockly.Msg.LIST_BACK_TOOLTIP = "Back to previous view.";
Blockly.Msg.LOGIC_BOOLEAN_FALSE = "falso";
Blockly.Msg.LOGIC_BOOLEAN_HELPURL = "https://github.com/google/blockly/wiki/Logic#values";
Blockly.Msg.LOGIC_BOOLEAN_TOOLTIP = "Restituisce vero o falso.";
Blockly.Msg.LOGIC_BOOLEAN_TRUE = "vero";
Blockly.Msg.LOGIC_COMPARE_HELPURL = "https://it.wikipedia.org/wiki/Disuguaglianza";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_EQ = "Restituisce vero se gli input sono uno uguale all'altro.";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_GT = "Restituisce vero se il primo input è maggiore o uguale al secondo.";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_GTE = "Restituisce uguale se il primo input è maggiore o uguale al secondo input.";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_LT = "Restituisce vero se il primo input è minore del secondo.";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_LTE = "Restituisce vero se il primo input è minore o uguale al secondo.";
Blockly.Msg.LOGIC_COMPARE_TOOLTIP_NEQ = "Restituisce vero se gli input non sono uno uguale all'altro.";
Blockly.Msg.LOGIC_NEGATE_HELPURL = "https://github.com/google/blockly/wiki/Logic#not";
Blockly.Msg.LOGIC_NEGATE_TITLE = "non %1";
Blockly.Msg.LOGIC_NEGATE_TOOLTIP = "Restituisce vero se l'input è falso. Restituisce falso se l'input è vero.";
Blockly.Msg.LOGIC_NULL = "nullo";
Blockly.Msg.LOGIC_NULL_HELPURL = "https://en.wikipedia.org/wiki/Nullable_type";
Blockly.Msg.LOGIC_NULL_TOOLTIP = "Restituisce il valore nullo.";
Blockly.Msg.LOGIC_OPERATION_AND = "e";
Blockly.Msg.LOGIC_OPERATION_HELPURL = "https://github.com/google/blockly/wiki/Logic#logical-operations";
Blockly.Msg.LOGIC_OPERATION_OR = "o";
Blockly.Msg.LOGIC_OPERATION_TOOLTIP_AND = "Restituisce vero se entrambi gli input sono veri.";
Blockly.Msg.LOGIC_OPERATION_TOOLTIP_OR = "Restituisce vero se almeno uno degli input è vero.";
Blockly.Msg.LOGIC_TERNARY_CONDITION = "test";
Blockly.Msg.LOGIC_TERNARY_HELPURL = "https://en.wikipedia.org/wiki/%3F:";
Blockly.Msg.LOGIC_TERNARY_IF_FALSE = "se falso";
Blockly.Msg.LOGIC_TERNARY_IF_TRUE = "se vero";
Blockly.Msg.LOGIC_TERNARY_TOOLTIP = "Verifica la condizione in 'test'. Se questa è vera restituisce il valore 'se vero' altrimenti restituisce il valore 'se falso'.";
Blockly.Msg.LOOP = "ripeti finché";
Blockly.Msg.LOOPFOREVER_TOOLTIP = "Ripete una azione indefinitamente.";
Blockly.Msg.LOOP_FOREVER = "ripeti senza fermarti";
Blockly.Msg.MAKEBLOCKBRICK_TOOLTIP = "Represents the MakeBlock brick with connected actors and sensors. There are also inbuilt actors and sensors available, e.g. buttons, display ...";
Blockly.Msg.MATH_ADDITION_SYMBOL = "+";
Blockly.Msg.MATH_ARITHMETIC_HELPURL = "https://it.wikipedia.org/wiki/Aritmetica";
Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_ADD = "Restituisce la somma dei due numeri.";
Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_DIVIDE = "Restituisce il quoziente dei due numeri.";
Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_MINUS = "Restituisce la differenza dei due numeri.";
Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_MULTIPLY = "Restituisce il prodotto dei due numeri.";
Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_POWER = "Restituisce il primo numero elevato alla potenza del secondo numero.";
Blockly.Msg.MATH_CHANGE_HELPURL = "https://it.wikipedia.org/wiki/Addizione";
Blockly.Msg.MATH_CHANGE_TITLE = "incrementa %1 di %2";
Blockly.Msg.MATH_CHANGE_TOOLTIP = "Aggiunge un numero alla variabile '%1'.";
Blockly.Msg.MATH_CONSTANT_HELPURL = "https://it.wikipedia.org/wiki/Costante_matematica";
Blockly.Msg.MATH_CONSTANT_TOOLTIP = "Restituisce una delle costanti comuniː π (3.141…), e (2.718…), φ (1.618…), radq(2) (1.414…), radq(½) (0.707…) o ∞ (infinito).";
Blockly.Msg.MATH_CONSTRAIN_HELPURL = "https://en.wikipedia.org/wiki/Clamping_%28graphics%29";
Blockly.Msg.MATH_CONSTRAIN_TITLE = "costringi %1 a stare tra %2 e %3";
Blockly.Msg.MATH_CONSTRAIN_TOOLTIP = "Costringe un numero all'interno dei limiti indicati (compresi), riportando i valori troppo grandi/piccoli al limite massimo/minimo.";
Blockly.Msg.MATH_DIVISION_SYMBOL = "÷";
Blockly.Msg.MATH_IS_DIVISIBLE_BY = "è divisibile per";
Blockly.Msg.MATH_IS_EVEN = "è pari";
Blockly.Msg.MATH_IS_NEGATIVE = "è negativo";
Blockly.Msg.MATH_IS_ODD = "è dispari";
Blockly.Msg.MATH_IS_POSITIVE = "è positivo";
Blockly.Msg.MATH_IS_PRIME = "è primo";
Blockly.Msg.MATH_IS_TOOLTIP = "Verifica se un numero è pari, dispari, primo, intero, positivo, negativo o se è divisibile per un certo numero. Restituisce vero o falso.";
Blockly.Msg.MATH_IS_WHOLE = "è intero";
Blockly.Msg.MATH_MODULO_HELPURL = "https://it.wikipedia.org/wiki/Resto";
Blockly.Msg.MATH_MODULO_TITLE = "resto di %1 ÷ %2";
Blockly.Msg.MATH_MODULO_TOOLTIP = "Restituisce il resto della divisione di due numeri.";
Blockly.Msg.MATH_MULTIPLICATION_SYMBOL = "×";
Blockly.Msg.MATH_NUMBER_HELPURL = "https://it.wikipedia.org/wiki/Numero";
Blockly.Msg.MATH_NUMBER_TOOLTIP = "Un numero.";
Blockly.Msg.MATH_ONLIST_HELPURL = "";
Blockly.Msg.MATH_ONLIST_OPERATOR_AVERAGE = "media della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_MAX = "massimo della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_MEDIAN = "mediana della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_MIN = "minimo della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_MODE = "mode della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_RANDOM = "elemento casuale della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_STD_DEV = "deviazione standard della lista";
Blockly.Msg.MATH_ONLIST_OPERATOR_SUM = "somma la lista";
Blockly.Msg.MATH_ONLIST_TOOLTIP_AVERAGE = "Restituisce la media (media aritmetica) dei valori numerici nella lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_MAX = "Restituisce il più grande numero della lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_MEDIAN = "Restituisce il valore mediano della lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_MIN = "Restituisce il più piccolo numero della lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_MODE = "Restituisce una lista degli elementi più frequenti nella lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_RANDOM = "Restituisce un elemento casuale della lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_STD_DEV = "Restituisce la deviazione standard della lista.";
Blockly.Msg.MATH_ONLIST_TOOLTIP_SUM = "Restituisce la somma di tutti i numeri nella lista.";
Blockly.Msg.MATH_POWER_SYMBOL = "^";
Blockly.Msg.MATH_RANDOM_FLOAT_HELPURL = "https://it.wikipedia.org/wiki/Numeri_pseudo-casuali";
Blockly.Msg.MATH_RANDOM_FLOAT_TITLE_RANDOM = "frazione casuale";
Blockly.Msg.MATH_RANDOM_FLOAT_TOOLTIP = "Restituisce una frazione compresa fra 0.0 (incluso) e 1.0 (escluso).";
Blockly.Msg.MATH_RANDOM_INT_HELPURL = "https://it.wikipedia.org/wiki/Numeri_pseudo-casuali";
Blockly.Msg.MATH_RANDOM_INT_TITLE = "intero casuale compreso tra %1 e %2";
Blockly.Msg.MATH_RANDOM_INT_TOOLTIP = "Restituisce un numero intero casuale compreso tra i due limiti indicati (inclusi).";
Blockly.Msg.MATH_ROUND_HELPURL = "https://it.wikipedia.org/wiki/Arrotondamento";
Blockly.Msg.MATH_ROUND_OPERATOR_ROUND = "arrotonda";
Blockly.Msg.MATH_ROUND_OPERATOR_ROUNDDOWN = "arrotonda verso il basso";
Blockly.Msg.MATH_ROUND_OPERATOR_ROUNDUP = "arrotonda verso l'alto";
Blockly.Msg.MATH_ROUND_TOOLTIP = "Arrotonda un numero verso l'alto o verso il basso.";
Blockly.Msg.MATH_SINGLE_HELPURL = "https://it.wikipedia.org/wiki/Radice_quadrata";
Blockly.Msg.MATH_SINGLE_OP_ABSOLUTE = "assoluto";
Blockly.Msg.MATH_SINGLE_OP_ROOT = "radice quadrata";
Blockly.Msg.MATH_SINGLE_TOOLTIP_ABS = "Restituisce il valore assoluto del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_EXP = "Restituisce e elevato alla potenza del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_LN = "Restituisce il logaritmo naturale del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_LOG10 = "Restituisce il logaritmo in base 10 del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_NEG = "Restituisce l'opposto del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_POW10 = "Restituisce 10 elevato alla potenza del numero.";
Blockly.Msg.MATH_SINGLE_TOOLTIP_ROOT = "Restituisce la radice quadrata del numero.";
Blockly.Msg.MATH_SUBTRACTION_SYMBOL = "-";
Blockly.Msg.MATH_TRIG_ACOS = "acos";
Blockly.Msg.MATH_TRIG_ASIN = "asin";
Blockly.Msg.MATH_TRIG_ATAN = "atan";
Blockly.Msg.MATH_TRIG_COS = "cos";
Blockly.Msg.MATH_TRIG_HELPURL = "https://it.wikipedia.org/wiki/Funzione_trigonometrica";
Blockly.Msg.MATH_TRIG_SIN = "sin";
Blockly.Msg.MATH_TRIG_TAN = "tan";
Blockly.Msg.MATH_TRIG_TOOLTIP_ACOS = "Restituisce l'arco-coseno di un numero.";
Blockly.Msg.MATH_TRIG_TOOLTIP_ASIN = "Restituisce l'arco-seno di un numero.";
Blockly.Msg.MATH_TRIG_TOOLTIP_ATAN = "Restituisce l'arco-tangente di un numero.";
Blockly.Msg.MATH_TRIG_TOOLTIP_COS = "Restituisce il coseno di un angolo espresso in gradi (non radianti).";
Blockly.Msg.MATH_TRIG_TOOLTIP_SIN = "Restituisce il seno di un angolo espresso in gradi (non radianti).";
Blockly.Msg.MATH_TRIG_TOOLTIP_TAN = "Restituisce la tangente di un angolo espresso in gradi (non radianti).";
Blockly.Msg.ME = "Me";
Blockly.Msg.MENU_ABOUT = "a proposito ...";
Blockly.Msg.MENU_ATTACH = "allega ...";
Blockly.Msg.MENU_BEGINNER = "principiante";
Blockly.Msg.MENU_CHANGE = "cambia ...";
Blockly.Msg.MENU_CHECK = "controlla";
Blockly.Msg.MENU_CODE_DOWNLOAD_TOOLTIP = "Download the source code of your program on the computer";
Blockly.Msg.MENU_CODE_REFRESH_TOOLTIP = "Refresh the source code, if you have changed the NEPO Block's.";
Blockly.Msg.MENU_CONNECT = "connetti ...";
Blockly.Msg.MENU_CREATE_LINK = "create program link ...";
Blockly.Msg.MENU_DELETE_USER = "elimina utente ...";
Blockly.Msg.MENU_EDIT = "modifica";
Blockly.Msg.MENU_EDIT_TOOLTIP = "Modifica.";
Blockly.Msg.MENU_EV3 = "Preparazione EV3";
Blockly.Msg.MENU_EXPERT = "esperto";
Blockly.Msg.MENU_EXPORT_PROG = "esporta il programma";
Blockly.Msg.MENU_FAQ = "FAQ (risposte alle domande più comuni)";
Blockly.Msg.MENU_GALLERY = "gallery";
Blockly.Msg.MENU_GALLERY_TOOLTIP = "gallery";
Blockly.Msg.MENU_GENERAL = "aiuto generale";
Blockly.Msg.MENU_HELP = "aiuto";
Blockly.Msg.MENU_HELP_TOOLTIP = "Aiuto.";
Blockly.Msg.MENU_IMPORT_PROG = "importa programma";
Blockly.Msg.MENU_LANGUAGE = "lingue";
Blockly.Msg.MENU_LANGUAGE_TOOLTIP = "Lingue.";
Blockly.Msg.MENU_LIST = "lista ...";
Blockly.Msg.MENU_LIST_CONF = "mie configurazioni ...";
Blockly.Msg.MENU_LIST_PROG = "miei programmi ...";
Blockly.Msg.MENU_LIST_PROG_EXAMPLES = "programmi di esempio ...";
Blockly.Msg.MENU_LOGGING = "registrando";
Blockly.Msg.MENU_LOG_IN = "login ...";
Blockly.Msg.MENU_LOG_OUT = "logout";
Blockly.Msg.MENU_MESSAGE_DOWNLOAD = "Il tuo programma è stato scaricato correttamente.";
Blockly.Msg.MENU_NEW = "nuovo ...";
Blockly.Msg.MENU_PROGRAMMING = "programmare con NEPO";
Blockly.Msg.MENU_PROPERTIES = "proprietà";
Blockly.Msg.MENU_RIGHT_HELP_TOOLTIP = "Open/close help view";
Blockly.Msg.MENU_RIGHT_INFO_TOOLTIP = "Open/close info view";
Blockly.Msg.MENU_ROBOT = "robot";
Blockly.Msg.MENU_ROBOT_STATE_INFO = "info";
Blockly.Msg.MENU_ROBOT_STATE_TOOLTIP = "Robot info.";
Blockly.Msg.MENU_ROBOT_TOOLTIP = "Robot.";
Blockly.Msg.MENU_SAVE = "salva";
Blockly.Msg.MENU_SAVE_AS = "salva come ...";
Blockly.Msg.MENU_SHOW_AGAIN = "mostra di nuovo la pagina di benvenuto";
Blockly.Msg.MENU_SHOW_CODE = "mostra/nascondi il codice sorgente";
Blockly.Msg.MENU_SIM_BACK = "Indietro";
Blockly.Msg.MENU_SIM_DRAW = "Scena di disegno";
Blockly.Msg.MENU_SIM_IMPORT_TOOLTIP = "Upload your own simulation background image, it will be appended at the end of the background's list.";
Blockly.Msg.MENU_SIM_MATH = "Scena di matematica";
Blockly.Msg.MENU_SIM_RESCUE = "Scena di soccorso";
Blockly.Msg.MENU_SIM_ROBERTA = "Scena di Roberta";
Blockly.Msg.MENU_SIM_ROBOT_TOOLTIP = "mostra/nascondi i dettagli sul robot";
Blockly.Msg.MENU_SIM_SCENE_TOOLTIP = "cambia la scena";
Blockly.Msg.MENU_SIM_SIMPLE = "Scena Semplice";
Blockly.Msg.MENU_SIM_START_TOOLTIP = "Start your program in the simulation.";
Blockly.Msg.MENU_SIM_STOP = "Stop";
Blockly.Msg.MENU_SIM_STOP_TOOLTIP = "Stop your program in the simulation.";
Blockly.Msg.MENU_SIM_VALUES_TOOLTIP = "Open/close the sensors' data view.";
Blockly.Msg.MENU_SIM_WRO = "World Robot Olympiad scene";
Blockly.Msg.MENU_START_BRICK = "esegui su »$«";
Blockly.Msg.MENU_START_SIM = "mostra/nascondi la finestra di simulazione";
Blockly.Msg.MENU_STATE_INFO = "informazioni sullo stato";
Blockly.Msg.MENU_TOOLBOX = "blocchi NEPO";
Blockly.Msg.MENU_TOOLBOX_BEGINNER = "blocchi NEPO per principianti";
Blockly.Msg.MENU_TOOLBOX_EXPERT = "blocchi NEPO per esperti";
Blockly.Msg.MENU_USER = "login";
Blockly.Msg.MENU_USER_STATE_TOOLTIP = "Informazioni dell'utente";
Blockly.Msg.MENU_USER_TOOLTIP = "Utente.";
Blockly.Msg.MENU_ZOOM = "ingrandimento";
Blockly.Msg.MENU_ZOOM_IN = "ingrandisci";
Blockly.Msg.MENU_ZOOM_OUT = "riduci zoom";
Blockly.Msg.MENU_ZOOM_RESET = "azzera ingrandimento";
Blockly.Msg.MESSAGE_ADDED_USER = "L'Utente »$« è stato aggiunto";
Blockly.Msg.MESSAGE_CONFIGURATION_DELETED = "La configurazione '$' è stata eliminata";
Blockly.Msg.MESSAGE_EDIT_CHECK = "Il tuo programma è stato controllato!";
Blockly.Msg.MESSAGE_EDIT_SAVE_CONFIGURATION = "La tua configurazione è stata salvata";
Blockly.Msg.MESSAGE_EDIT_SAVE_CONFIGURATION_AS = "La tua configurazione è stata salvata come '$'";
Blockly.Msg.MESSAGE_EDIT_SAVE_GROUP_AS = "Your group has been created";
Blockly.Msg.MESSAGE_EDIT_SAVE_PROGRAM = "Il tuo programma è stato salvato";
Blockly.Msg.MESSAGE_EDIT_SAVE_PROGRAM_AS = "Il tuo programma è stato salvato come '$'";
Blockly.Msg.MESSAGE_EDIT_START = "Il tuo programma '$' sarà esequito fra un momento!";
Blockly.Msg.MESSAGE_FIRMWARE_ERROR = "C'è un conflitto tra la versione della firmware del tuo robot e il Open Roberta Lab. Ci contatti per favore.";
Blockly.Msg.MESSAGE_GROUP_DELETED = "Group »$« was deleted";
Blockly.Msg.MESSAGE_INVALID_NAME = "Inserisci un nome corretto per favore. Un nome corretto comincia con una lettera e puo solo contenere lettere o numeri.";
Blockly.Msg.MESSAGE_NOT_AVAILABLE = "Non è disponibile.";
Blockly.Msg.MESSAGE_PROGRAM_DELETED = "Il programma '$' è stato cancellato";
Blockly.Msg.MESSAGE_RESTART_ROBOT = "Per favore riconneti il robot all'Open Roberta Lab.";
Blockly.Msg.MESSAGE_ROBOT_CONNECTED = "Il tuo robot »$« è connesso";
Blockly.Msg.MESSAGE_ROBOT_DISCONNECTED = "An active robot was disconnected";
Blockly.Msg.MESSAGE_USER_DELETED = "Utente eliminato";
Blockly.Msg.MESSAGE_USER_GROUP_DELETED = "L'utente »$« è stato eliminato";
Blockly.Msg.MESSAGE_USER_LOGIN = "Ciao »$«";
Blockly.Msg.MESSAGE_USER_LOGOUT = "Sei uscito";
Blockly.Msg.MICROBITBRICK_TOOLTIP = "Represents micro:bit, a pocket-sized codeable computer. There are also inbuilt actors and sensors available, e.g. buttons, display ...";
Blockly.Msg.MICROPHONE_GETSAMPLE_TOOLTIP = "Gets the current reading from the microphone in % (mapped to 0 - 100). If the value is always low, the value has to be multiplied by 10, because the amplification is missing on the hardware.";
Blockly.Msg.MODE = "modalità";
Blockly.Msg.MODE_ACCELERATION = "accelerazione";
Blockly.Msg.MODE_AMBIENTLIGHT = "luce ambiente";
Blockly.Msg.MODE_ANGLE = "angolo";
Blockly.Msg.MODE_CLOSE = "close";
Blockly.Msg.MODE_COLOUR = "colore";
Blockly.Msg.MODE_DEGREE = "grado";
Blockly.Msg.MODE_DISTANCE = "distanza";
Blockly.Msg.MODE_LIGHT = "luce";
Blockly.Msg.MODE_OBSTACLE = "ostacolo";
Blockly.Msg.MODE_OPEN = "open";
Blockly.Msg.MODE_ORIENTATION = "orientamento";
Blockly.Msg.MODE_PRESENCE = "presenza";
Blockly.Msg.MODE_RATE = "rapporto";
Blockly.Msg.MODE_RGB = "RGB (rosso, verde, blu)";
Blockly.Msg.MODE_ROTATION = "rotazione";
Blockly.Msg.MODE_SOUND = "suono";
Blockly.Msg.MOTOR = "motore";
Blockly.Msg.MOTORDIFF_ON_FOR_TOOLTIP = "Avvia il robot con velocità indicata e si ferma dopo una distanza indicata.";
Blockly.Msg.MOTORDIFF_ON_TOOLTIP = "Avvia il robot con la velocità indicata.";
Blockly.Msg.MOTORDIFF_STOP_TOOLTIP = "Ferma il robot.";
Blockly.Msg.MOTORDIFF_TURN_FOR_TOOLTIP = "Gira il robot di un dato numero di gradi.";
Blockly.Msg.MOTORDIFF_TURN_TOOLTIP = "Gira il robot.";
Blockly.Msg.MOTOR_ARDU_TOOLTIP = "Represents a Bot'n Roll chassis motor.";
Blockly.Msg.MOTOR_BACKWARD = "indietro";
Blockly.Msg.MOTOR_BIG = "grande";
Blockly.Msg.MOTOR_BIG_TOOLTIP = "Rappresenta un motore grande.";
Blockly.Msg.MOTOR_BRAKE = "freno";
Blockly.Msg.MOTOR_DEGREE = "di gradi";
Blockly.Msg.MOTOR_DISTANCE = "distanza (cm)";
Blockly.Msg.MOTOR_DRIVE = "muovi";
Blockly.Msg.MOTOR_FLOAT = "motore libero";
Blockly.Msg.MOTOR_FOREWARD = "in avanti";
Blockly.Msg.MOTOR_GEARED_TOOLTIP = "Represents a geared motor.";
Blockly.Msg.MOTOR_GETPOWER_TOOLTIP = "Ottiene la potenza di questo motore.";
Blockly.Msg.MOTOR_LEFT = "a sinistra";
Blockly.Msg.MOTOR_MIDDLE = "medio";
Blockly.Msg.MOTOR_MIDDLE_TOOLTIP = "Rappresenta un motore centrale.";
Blockly.Msg.MOTOR_NONE = "nessuno/a";
Blockly.Msg.MOTOR_ON_FOR_TOOLTIP = "Accende il motore e poi lo ferma dopo una rotazione di un certo numero di gradi.";
Blockly.Msg.MOTOR_ON_TOOLTIP = "Accende il motore con la potenza indicata.";
Blockly.Msg.MOTOR_OTHER = "other power consumer";
Blockly.Msg.MOTOR_PAN = "pan";
Blockly.Msg.MOTOR_PORT = "motore collegato alla porta";
Blockly.Msg.MOTOR_REGULATION = "regolazione";
Blockly.Msg.MOTOR_RIGHT = "a destra";
Blockly.Msg.MOTOR_ROTATION = "giri";
Blockly.Msg.MOTOR_ROTATION_REVERSE = "direzione di rotazione";
Blockly.Msg.MOTOR_SETPOWER_TOOLTIP = "Imposta la potenza di questo motore.";
Blockly.Msg.MOTOR_SIDE = "lato";
Blockly.Msg.MOTOR_SPEED = "velocità %";
Blockly.Msg.MOTOR_SPEED_0 = "La velocità del motore è 0!";
Blockly.Msg.MOTOR_STEER = "muovi sterzando";
Blockly.Msg.MOTOR_STOP = "stop/ferma";
Blockly.Msg.MOTOR_STOP_TOOLTIP = "Ferma questo motore.";
Blockly.Msg.MOTOR_TILT = "inclinazione";
Blockly.Msg.MOTOR_TURN = "ruota";
Blockly.Msg.NAO_ABSOLUTE = "absolute";
Blockly.Msg.NAO_ACCELEROMETER = "accelerometer";
Blockly.Msg.NAO_ACCELEROMETER_TOOLTIP = "Get the current reading from the accelerometer in the given direction";
Blockly.Msg.NAO_ANIMATION_TOOLTIP = "Perform the selected animation.";
Blockly.Msg.NAO_ANSWER = "answer";
Blockly.Msg.NAO_APPLYPOSTURE = "apply posture";
Blockly.Msg.NAO_APPLYPOSTURE_TOOLTIP = "Robot goes into the selected posture.";
Blockly.Msg.NAO_AUTONOMOUS = "autonomous life";
Blockly.Msg.NAO_AUTONOMOUS_TOOLTIP = "Toggle the state of Autonomous life. Turn Autonomous life off to gain more control over NAOs behaviour.";
Blockly.Msg.NAO_BLINK = "blink";
Blockly.Msg.NAO_CAMERA = "camera";
Blockly.Msg.NAO_CAMERA_BOTTOM = "bottom";
Blockly.Msg.NAO_CAMERA_TOP = "top";
Blockly.Msg.NAO_CHAT_TOOLTIP = "Add a reaction if a word is recognized.";
Blockly.Msg.NAO_CURRENT = "electric current of";
Blockly.Msg.NAO_DETECTFACE = "detect face";
Blockly.Msg.NAO_DETECTFACE_TOOLTIP = "Detect a face previously saved under a given name";
Blockly.Msg.NAO_DIALOG_TOOLTIP = "The robot tries to recognize the phrase and answers on success.";
Blockly.Msg.NAO_FILENAME = "filename";
Blockly.Msg.NAO_FORGETFACEOF = "forget face of";
Blockly.Msg.NAO_FORGETFACE_TOOLTIP = "Forget a face previously saved under a given name";
Blockly.Msg.NAO_FRAME = "frame";
Blockly.Msg.NAO_FRAME_TORSO = "torso";
Blockly.Msg.NAO_FRAME_WORLD = "world";
Blockly.Msg.NAO_FSR = "force sensitive resistor";
Blockly.Msg.NAO_FSR_TOOLTIP = "Get the current reading from the force sensitive resistor under the feet of the robot.";
Blockly.Msg.NAO_GETCURRENT_TOOLTIP = "Get the electric current from the selected joint.";
Blockly.Msg.NAO_GETLANGUAGE = "get language";
Blockly.Msg.NAO_GETLANGUAGE_TOOLTIP = "Get the active language.";
Blockly.Msg.NAO_GETVOLUME_TOOLTIP = "Get the volume.";
Blockly.Msg.NAO_GYROMETER = "gyrometer";
Blockly.Msg.NAO_GYROMETER_TOOLTIP = "Get the current reading from the gyrometer in the given direction.";
Blockly.Msg.NAO_HAND = "hand";
Blockly.Msg.NAO_HAND_TOOLTIP = "Open/close a hand.";
Blockly.Msg.NAO_HEADSENSOR = "head sensor";
Blockly.Msg.NAO_INTENSITY = "intensity";
Blockly.Msg.NAO_JOINT_ANKLEPITCH = "ankle pitch";
Blockly.Msg.NAO_JOINT_ANKLEROLL = "ankle roll";
Blockly.Msg.NAO_JOINT_ELBOWROLL = "elbow roll";
Blockly.Msg.NAO_JOINT_ELBOWYAW = "elbow yaw";
Blockly.Msg.NAO_JOINT_HAND = "hand";
Blockly.Msg.NAO_JOINT_HEADPITCH = "head pitch";
Blockly.Msg.NAO_JOINT_HEADYAW = "head yaw";
Blockly.Msg.NAO_JOINT_HIPPITCH = "hip pitch";
Blockly.Msg.NAO_JOINT_HIPROLL = "hip roll";
Blockly.Msg.NAO_JOINT_HIPYAWPITCH = "hip yaw pitch";
Blockly.Msg.NAO_JOINT_KNEEPITCH = "knee pitch";
Blockly.Msg.NAO_JOINT_SHOULDERPITCH = "shoulder pitch";
Blockly.Msg.NAO_JOINT_SHOULDERROLL = "shoulder roll";
Blockly.Msg.NAO_JOINT_WRISTYAW = "wrist yaw";
Blockly.Msg.NAO_LANGUAGE_ARABIC = "Arabic";
Blockly.Msg.NAO_LANGUAGE_BRAZILIAN = "Brazilian";
Blockly.Msg.NAO_LANGUAGE_CHINESE = "Chinese";
Blockly.Msg.NAO_LANGUAGE_CZECH = "Czech";
Blockly.Msg.NAO_LANGUAGE_DANISH = "Dansk";
Blockly.Msg.NAO_LANGUAGE_DUTCH = "Dutch";
Blockly.Msg.NAO_LANGUAGE_ENGLISH = "English";
Blockly.Msg.NAO_LANGUAGE_FINNISH = "Suomi";
Blockly.Msg.NAO_LANGUAGE_FRENCH = "Français";
Blockly.Msg.NAO_LANGUAGE_GERMAN = "Deutsch";
Blockly.Msg.NAO_LANGUAGE_GREEK = "Greek";
Blockly.Msg.NAO_LANGUAGE_ITALIAN = "Italiano";
Blockly.Msg.NAO_LANGUAGE_JAPANESE = "Japanese";
Blockly.Msg.NAO_LANGUAGE_KOREAN = "Korean";
Blockly.Msg.NAO_LANGUAGE_NORWEGIAN = "Norsk";
Blockly.Msg.NAO_LANGUAGE_POLISH = "Polski";
Blockly.Msg.NAO_LANGUAGE_PORTUGUESE = "Português";
Blockly.Msg.NAO_LANGUAGE_RUSSIAN = "Українська";
Blockly.Msg.NAO_LANGUAGE_SPANISH = "Español";
Blockly.Msg.NAO_LANGUAGE_SWEDISH = "Svensk";
Blockly.Msg.NAO_LANGUAGE_TURKISH = "Türkçe";
Blockly.Msg.NAO_LEARNFACEOF = "learn face of";
Blockly.Msg.NAO_LEARNFACE_TOOLTIP = "Learn and save a face under a given name";
Blockly.Msg.NAO_LED = "led";
Blockly.Msg.NAO_LEDOFF_TOOLTIP = "Turn the LEDs off.";
Blockly.Msg.NAO_LEDRESET_TOOLTIP = "Reset the LEDs.";
Blockly.Msg.NAO_LED_ALL = "all";
Blockly.Msg.NAO_LED_CHEST = "chest";
Blockly.Msg.NAO_LED_EAR = "ear";
Blockly.Msg.NAO_LED_EARS = "ears";
Blockly.Msg.NAO_LED_EYE = "eye";
Blockly.Msg.NAO_LED_EYES = "eyes";
Blockly.Msg.NAO_LED_FOOT = "foot";
Blockly.Msg.NAO_LED_HEAD = "head";
Blockly.Msg.NAO_LED_TOOLTIP = "Set the color of a group of LEDs.";
Blockly.Msg.NAO_LOOKAT = "look at";
Blockly.Msg.NAO_MODE_ACTIVE = "active";
Blockly.Msg.NAO_MODE_REST = "rest";
Blockly.Msg.NAO_MODE_SIT = "sit";
Blockly.Msg.NAO_MODE_TOOLTIP = "Turns the stiffness of the robot's motors on and goes to a standing posture, or goes to a resting posture and turns the stiffness off.";
Blockly.Msg.NAO_MOVE = "move";
Blockly.Msg.NAO_MOVEJOINT_TOOLTIP = "Move a single joint.";
Blockly.Msg.NAO_NAOMARK = "detect NAO mark";
Blockly.Msg.NAO_NAOMARK_TOOLTIP = "Get the number of a detected Nao Mark.";
Blockly.Msg.NAO_PART_ARM = "arm";
Blockly.Msg.NAO_PART_ARMS = "arms";
Blockly.Msg.NAO_PART_BODY = "body";
Blockly.Msg.NAO_PART_HEAD = "head";
Blockly.Msg.NAO_PART_LEG = "leg";
Blockly.Msg.NAO_PART_LEGS = "legs";
Blockly.Msg.NAO_PERFORM = "perform";
Blockly.Msg.NAO_PHRASE = "phrase";
Blockly.Msg.NAO_PLAYFILE_TOOLTIP = "Plays a sound file from the robot.";
Blockly.Msg.NAO_PLAY_FILE = "play file";
Blockly.Msg.NAO_POINTAT = "point at";
Blockly.Msg.NAO_POINTLOOKAT_TOOLTIP = "Rbbot points or looks at a given position";
Blockly.Msg.NAO_POSTURE_CROUCH = "Crouch";
Blockly.Msg.NAO_POSTURE_LYINGBACK = "LyingBack";
Blockly.Msg.NAO_POSTURE_LYINGBELLY = "LyingBelly";
Blockly.Msg.NAO_POSTURE_SITRELAX = "SitRelax";
Blockly.Msg.NAO_POSTURE_STAND = "Stand";
Blockly.Msg.NAO_POSTURE_STANDINIT = "StandInit";
Blockly.Msg.NAO_POSTURE_STANDZERO = "StandZero";
Blockly.Msg.NAO_QQVGA = "160*120";
Blockly.Msg.NAO_QVGA = "320*240";
Blockly.Msg.NAO_RANDOMEYES = "random eyes";
Blockly.Msg.NAO_RANDOMEYES_TOOLTIP = "The color of the eyes is changed randomly for a specified amount of time.";
Blockly.Msg.NAO_RASTA = "rasta";
Blockly.Msg.NAO_RASTA_TOOLTIP = "The color of the eyes is changed between green, yellow and red for a specified amount of time.";
Blockly.Msg.NAO_RECOGNIZEDWORD = "recognized word";
Blockly.Msg.NAO_RECOGNIZEDWORD_TOOLTIP = "Returns the last word the robot recognized.";
Blockly.Msg.NAO_RECOGNIZEWORD = "recognize word from";
Blockly.Msg.NAO_RECOGNIZEWORDOR = "or recognize word";
Blockly.Msg.NAO_RECOGNIZEWORD_TOOLTIP = "Make NAO recognize a word or a phrase from the provided list";
Blockly.Msg.NAO_RECORDVIDEO = "record video";
Blockly.Msg.NAO_RECORDVIDEO_TOOLTIP = "Records a video and saves it on the robot.";
Blockly.Msg.NAO_RELATIVE = "relative";
Blockly.Msg.NAO_RESOLUTION = "resolution";
Blockly.Msg.NAO_SAY = "say";
Blockly.Msg.NAO_SAY_TOOLTIP = "The robot says the given text.";
Blockly.Msg.NAO_SETINTENSITY_TOOLTIP = "Set the intensity of a group of LEDs.";
Blockly.Msg.NAO_SETLANGUAGE = "set language";
Blockly.Msg.NAO_SETLANGUAGE_TOOLTIP = "Set the language.";
Blockly.Msg.NAO_SETVOLUME_TOOLTIP = "Set the volume.";
Blockly.Msg.NAO_STIFFNESS = "stiffness of";
Blockly.Msg.NAO_STIFFNESS_TOOLTIP = "The stiffness of the selected body part is turned on or off";
Blockly.Msg.NAO_STOP = "stop movement";
Blockly.Msg.NAO_STOP_TOOLTIP = "The robot stops all movement.";
Blockly.Msg.NAO_TAI_CHI = "tai chi";
Blockly.Msg.NAO_TAKEPICTURE = "take picture";
Blockly.Msg.NAO_TAKEPICTURE_TOOLTIP = "Takess a picture and save it on the robot.";
Blockly.Msg.NAO_TOUCHSENSOR_TOOLTIP = "Is true if touch sensor on the robot is touched.";
Blockly.Msg.NAO_TOUCH_BUMPER = "bumper";
Blockly.Msg.NAO_TOUCH_FRONT = "front";
Blockly.Msg.NAO_TOUCH_HAND = "hand";
Blockly.Msg.NAO_TOUCH_REAR = "rear";
Blockly.Msg.NAO_TURN_TOOLTIP = "Turns the robot for number of degrees";
Blockly.Msg.NAO_VGA = "640*480";
Blockly.Msg.NAO_WALK = "walk";
Blockly.Msg.NAO_WALKTO = "walk to";
Blockly.Msg.NAO_WALKTO_TOOLTIP = "The robot walks to the give position.";
Blockly.Msg.NAO_WALK_ASYNC_TOOLTIP = "Makes the robot walk infinitely";
Blockly.Msg.NAO_WALK_TOOLTIP = "Makes the robot walk a distance.";
Blockly.Msg.NAO_WAVE = "wave";
Blockly.Msg.NAO_WIPE_FOREHEAD = "wipe forehead";
Blockly.Msg.NEW_VARIABLE = "Nuova variabile...";
Blockly.Msg.NEW_VARIABLE_TITLE = "Nome della nuova variabile:";
Blockly.Msg.NO = "no";
Blockly.Msg.NXTBRICK_TOOLTIP = "Represents the NXT brick with connected actors and sensors. There are also inbuilt actors and sensors available, e.g. buttons, display ...";
Blockly.Msg.OFF = "spento/a";
Blockly.Msg.OLDER_THEN_14 = "I am older then 14!";
Blockly.Msg.ON = "acceso/a";
Blockly.Msg.ORA_ACCOUNT_NOT_ACTIVATED_TO_SHARE = "Your are not allowed to share. Please activate your account!";
Blockly.Msg.ORA_COMMAND_INVALID = "Comando Javascript non valido.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_CONFIGURATION_NOT_FOUND = "Non è stato possibile trovare la configurazione del robot sul server.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_CONFIGURATION_TRANSFORM_FAILED = "Non è stato possibile trasformare la configurazione del robot nel linguaggio di programmazione del robot.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_PROGRAM_COMPILE_FAILED = "Non è stato possibile trasformare il programma in codice macchina.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_PROGRAM_GENERATION_FAILED = "Non è stato possibile generare il programma.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_PROGRAM_NOT_FOUND = "Non è stato possibile trasformare il programma nel linguaggio di programmazione del robot.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_PROGRAM_STORE_FAILED = "Non è stato possibile salvare il programma a causa di un errore interno.";
Blockly.Msg.ORA_COMPILERWORKFLOW_ERROR_PROGRAM_TRANSFORM_FAILED = "Non è stato possibile trasformare il programma nel linguaggio di programmazione del robot.";
Blockly.Msg.ORA_CONFIGURATION_DELETE_ERROR = "Si è verificato un errore durante la eliminazione della configurazione del robot, per favore riprovaci!";
Blockly.Msg.ORA_CONFIGURATION_DELETE_SUCCESS = "Configurazione eliminata";
Blockly.Msg.ORA_CONFIGURATION_ERROR_ID_INVALID = "Il nome della configurazione non è un identifiactore valido.";
Blockly.Msg.ORA_CONFIGURATION_GET_ALL_SUCCESS = "Configurazione caricata";
Blockly.Msg.ORA_CONFIGURATION_GET_ONE_ERROR_NOT_FOUND = "Non è stato possibile trovare la configurazione del robot nel database.";
Blockly.Msg.ORA_CONFIGURATION_GET_ONE_SUCCESS = "Configurazione caricata";
Blockly.Msg.ORA_CONFIGURATION_SAVE_ERROR = "Si è verificato un errore durante il salvataggio della configurazione del robot.";
Blockly.Msg.ORA_CONFIGURATION_SAVE_ERROR_NOT_SAVED_TO_DB = "Si è verificato un errore durante il salvataggio della configurazione nel database.";
Blockly.Msg.ORA_CONFIGURATION_SAVE_SUCCESS = "Configurazione salvata";
Blockly.Msg.ORA_GROUP_CREATE_ERROR_NOT_SAVED_TO_DB = "This group already exists";
Blockly.Msg.ORA_OWNER_DOES_NOT_EXIST = "Questo proprietario (utente?) non esiste.";
Blockly.Msg.ORA_PROGRAM_CONFIGURATION_NOT_COMPATIBLE = "Questo programma non è compatibile con la configurazione";
Blockly.Msg.ORA_PROGRAM_DELETE_ERROR = "Si è verificato un errore durante la eliminazione della configurazione. Per favor riprovaci!";
Blockly.Msg.ORA_PROGRAM_DELETE_SUCCESS = "Programma eliminato";
Blockly.Msg.ORA_PROGRAM_ERROR_ID_INVALID = "Il nome del tuo programma è già stato usato dal sistema. Per favore prova di nuovo con un nome diverso!";
Blockly.Msg.ORA_PROGRAM_GET_ALL_SUCCESS = "Programmi caricati";
Blockly.Msg.ORA_PROGRAM_GET_ONE_ERROR_NOT_FOUND = "Il programma non è stato trovato.";
Blockly.Msg.ORA_PROGRAM_GET_ONE_ERROR_NOT_LOGGED_IN = "Non sei autenticato/connesso/registrato?";
Blockly.Msg.ORA_PROGRAM_GET_ONE_SUCCESS = "Programma caricato";
Blockly.Msg.ORA_PROGRAM_IMPORT_ERROR = "$.xml non è un programma NEPO valido e non puo essere caricato.";
Blockly.Msg.ORA_PROGRAM_IMPORT_ERROR_WRONG_ROBOT_TYPE = "You are trying to import a program for a robot of a different type as selected in the menu. <br> Please switch the robot type in the robot's menu and try to import again!";
Blockly.Msg.ORA_PROGRAM_INVALID_STATEMETNS = "There are errors in your program. Please check the messages";
Blockly.Msg.ORA_PROGRAM_SAVE_AS_ERROR_PROGRAM_EXISTS = "Questo programma esiste già.";
Blockly.Msg.ORA_PROGRAM_SAVE_ERROR_NO_WRITE_PERMISSION = "Non hai i permessi per modificare questo programma!";
Blockly.Msg.ORA_PROGRAM_SAVE_ERROR_OPTIMISTIC_TIMESTAMP_LOCKING = "Questo programma è stato cambiato recentemente da qualcun'altro. Puoi salvare i tuoi cambiamenti in un nuovo programma, scegli >salva come< !";
Blockly.Msg.ORA_PROGRAM_SAVE_ERROR_PROGRAM_TO_UPDATE_NOT_FOUND = "Non è stato possibile trovare questo programma, quindi non è possibile aggiornarlo.";
Blockly.Msg.ORA_PROGRAM_SAVE_SUCCESS = "Programma salvato";
Blockly.Msg.ORA_PROGRAM_TO_SHARE_DOES_NOT_EXIST = "Il programma che vuoi condividere non esiste.";
Blockly.Msg.ORA_ROBOT_FIRMWAREUPDATE_IMPOSSIBLE = "Si è verificato un errore durante l'aggiornamento della firmware sul tuo robot.";
Blockly.Msg.ORA_ROBOT_FIRMWAREUPDATE_POSSIBLE = "Il firmware è stato aggiornato";
Blockly.Msg.ORA_ROBOT_NOT_CONNECTED = "Nessun robot è connesso. Controlla, se il tuo robot è acceso, connettilo con il server e inserisci la password del robot sotto 'robot' - connetti nel menu. Se hai problemi con questo passo, dai un'occhiata a 'aiuto'.";
Blockly.Msg.ORA_ROBOT_NOT_WAITING = "Il robot non aspetta un comando di esecuzione.";
Blockly.Msg.ORA_ROBOT_PUSH_RUN = "Il robot ha aspettato e ora il programma viene caricato sul robot";
Blockly.Msg.ORA_ROBOT_SET_SUCCESS = "Robot selezionato!";
Blockly.Msg.ORA_SERVER_ERROR = "Errore del server";
Blockly.Msg.ORA_TOKEN_SET_ERROR_NO_ROBOT_WAITING = "Controlla, se il tuo robot è acceso e connesso al server. Se hai problemi con questo passo, dai un'occhiata a 'aiuto'.";
Blockly.Msg.ORA_TOKEN_SET_ERROR_WRONG_ROBOTTYPE = "You are trying to connect a robot of a different type as selected in the menu. <br> Please switch the robot type in the robot's menu and try to connect again!";
Blockly.Msg.ORA_TOKEN_SET_SUCCESS = "Token ricevuto";
Blockly.Msg.ORA_TOOLBOX_DELETE_ERROR = "Si è verificato un errore durante la eliminazione della toolbox, per favore riprovaci!";
Blockly.Msg.ORA_TOOLBOX_DELETE_SUCCESS = "Toolbox elimonata";
Blockly.Msg.ORA_TOOLBOX_ERROR_ID_INVALID = "Nome della toolbox non è un identificatore valido.";
Blockly.Msg.ORA_TOOLBOX_GET_ALL_SUCCESS = "Toolbox caricate";
Blockly.Msg.ORA_TOOLBOX_GET_ONE_ERROR_NOT_FOUND = "Non è stato possibile trovare la toolbox nel database.";
Blockly.Msg.ORA_TOOLBOX_GET_ONE_SUCCESS = "Toolbox caricata";
Blockly.Msg.ORA_TOOLBOX_SAVE_ERROR = "Si è verificato un errore durante il salvataggio della toolbox.";
Blockly.Msg.ORA_TOOLBOX_SAVE_ERROR_NOT_SAVED_TO_DB = "Si è verificato un errore durante il salvataggio della configurazione nel database.";
Blockly.Msg.ORA_TOOLBOX_SAVE_SUCCESS = "Toolbox salvata";
Blockly.Msg.ORA_TOOLBOX_TRANSFORM_ERROR = "Transformation error?";
Blockly.Msg.ORA_USER_ACTIVATION_SENT_MAIL_FAIL = "Sorry, we cannot send a mail to you, please contact »sterbini@di.uniroma1.it«";
Blockly.Msg.ORA_USER_ACTIVATION_SENT_MAIL_SUCCESS = "We send a mail to you, please check your mailbox!";
Blockly.Msg.ORA_USER_ACTIVATION_SUCCESS = "Activation of the user account is successful!";
Blockly.Msg.ORA_USER_CREATE_ERROR_ACCOUNT_LENGTH = "Your account name or your user name is to long. Please make sure that they are not longer then 25 digits. Your account couldn't be created.";
Blockly.Msg.ORA_USER_CREATE_ERROR_CONTAINS_SPECIAL_CHARACTERS = "You are using some one or more special characters in your account name! Please remove them. Your account couldn't be created.";
Blockly.Msg.ORA_USER_CREATE_ERROR_MISSING_REQ_FIELDS = "Controlla che tutti gli campi di input richiesti sono stati compilati.";
Blockly.Msg.ORA_USER_CREATE_ERROR_NOT_SAVED_TO_DB = "Il nome utente esiste già nel database, per favore scegline un altro.";
Blockly.Msg.ORA_USER_CREATE_SUCCESS = "L'utente »$« è stato creato!";
Blockly.Msg.ORA_USER_DELETE_ERROR_ID_NOT_FOUND = "Si è verificato un errore durante l'eliminazione dell'utente.";
Blockly.Msg.ORA_USER_DELETE_ERROR_NOT_DELETED_IN_DB = "Si è verificato un errore durante l'eliminazione dell'utente nel database.";
Blockly.Msg.ORA_USER_DELETE_SUCCESS = "L'utente »$« è stato eliminato. Spero di rivederti presto!";
Blockly.Msg.ORA_USER_EMAIL_ONE_ERROR_USER_NOT_EXISTS_WITH_THIS_EMAIL = "Questo indirizzo e-mail è sconosciuto, forse c'è un errore di battitura!";
Blockly.Msg.ORA_USER_ERROR_EMAIL_USED = "C'è già un account registrato con questa EMail. Inserisci un indirizzo EMail differente!";
Blockly.Msg.ORA_USER_GET_ALL_SUCCESS = "Untenti caricati";
Blockly.Msg.ORA_USER_GET_ONE_ERROR_ID_OR_PASSWORD_WRONG = "Il nome utente o la password sono errati. Prova di nuovo!";
Blockly.Msg.ORA_USER_GET_ONE_SUCCESS = "Login (accesso) riuscito";
Blockly.Msg.ORA_USER_GROUP_SAVE_AS_ERROR_USER_GROUP_EXISTS = "This user already belongs to the group";
Blockly.Msg.ORA_USER_PASSWORD_RECOVERY_EXPIRED_URL = "The link is not valid anymore. Please ask for resetting your password again, if you still cannot remember it!";
Blockly.Msg.ORA_USER_PASSWORD_RECOVERY_SENT_MAIL_FAIL = "Sorry, we cannot send a mail to you, please contact »sterbini@di.uniroma1.it«";
Blockly.Msg.ORA_USER_PASSWORD_RECOVERY_SENT_MAIL_SUCCESS = "We send a mail to you, please check your mailbox!";
Blockly.Msg.ORA_USER_TO_ADD_NOT_FOUND = "User was not found";
Blockly.Msg.ORA_USER_TO_SHARE_DOES_NOT_EXIST = "L'utente con cui vorresti condividere il programma non esiste. <br />Prova ancora.!";
Blockly.Msg.ORA_USER_TO_SHARE_SAME_AS_LOGIN_USER = "Non puoi condividere programmi con te stesso/a.";
Blockly.Msg.ORA_USER_UPDATE_SUCCESS = "La tua password è stata modificata con successo!";
Blockly.Msg.ORDINAL_NUMBER_SUFFIX = "";
Blockly.Msg.PIN_GETSAMPLE_ANALOG_TOOLTIP = "Returns the value from the specified analog pin. The value is between 0 and 1023.";
Blockly.Msg.PIN_GETSAMPLE_DIGITAL_TOOLTIP = "Returns the value from the specified digital pin. The value is either HIGH »1« or LOW »0«.";
Blockly.Msg.PIN_GETSAMPLE_PULSEHIGH_TOOLTIP = "Returns the pulse HIGH on a pin in microseconds or -1 if no complete pulse was received within the timeout.";
Blockly.Msg.PIN_GETSAMPLE_PULSELOW_TOOLTIP = "Returns the pulse LOW on a pin in microseconds or -1 if no complete pulse was received within the timeout.";
Blockly.Msg.PIN_ISTOUCHED_TOOLTIP = "Is the selected pin touched?";
Blockly.Msg.PIN_WRITE = "write";
Blockly.Msg.PIRMOTION_TOOLTIP = "Represents an PIR motion sensor.";
Blockly.Msg.PITCH = "pitch";
Blockly.Msg.PLAY = "suona";
Blockly.Msg.PLAY_DURATION = "durata (ms)";
Blockly.Msg.PLAY_EIGHTH = "eighth note";
Blockly.Msg.PLAY_FILE = "il file";
Blockly.Msg.PLAY_FILE_TOOLTIP = "Riproduce un file audio.";
Blockly.Msg.PLAY_FREQUENZ = "frequenza (Hz)";
Blockly.Msg.PLAY_GETVOLUME_TOOLTIP = "Ottiene volume attuale.";
Blockly.Msg.PLAY_HALF = "half note";
Blockly.Msg.PLAY_NOTE = "note";
Blockly.Msg.PLAY_NOTE_TOOLTIP = "Plays a music note";
Blockly.Msg.PLAY_QUARTER = "quarter note";
Blockly.Msg.PLAY_SETVOLUME_TOOLTIP = "Imposta il volume.";
Blockly.Msg.PLAY_SIXTEENTH = "sixteenth note";
Blockly.Msg.PLAY_TONE = "suono";
Blockly.Msg.PLAY_TONE_TOOLTIP = "Suona una nota.";
Blockly.Msg.PLAY_VOLUME = "volume";
Blockly.Msg.PLAY_WHOLE = "whole note";
Blockly.Msg.POPUP_ABOUT_JOIN = "Voglio aiutare";
Blockly.Msg.POPUP_ABOUT_TEXT = "Lo Open Roberta Lab è un ambiente di programmazione integrato, basato su cloud, che permette a bambini e adolescenti di imparare a programmare facilmente con robot di Lego Mindstorms EV3. Questa piattaforma é completamente open source (fonte aperta), quindi la partecipazione è vantaggiosa! La software e gli strumenti di sviluppo open source (a fonte aperta) sono disponibili tramite i server di Fraunhofer.";
Blockly.Msg.POPUP_ABOUT_TEXT_DEV = "Lo Open Roberta Lab è una piattaforma a sorgente aperto (open source) di programmazione, sviluppata dal Fraunhofer IAIS nell'ambito della initiativa <a href='http://www.roberta-home.de/'>»Roberta – Learning with Robots«</a>";
Blockly.Msg.POPUP_ABOUT_TEXT_GOOG = "L'iniziativa Open Roberta è stata avviata in collaborazione con Google Germany per riducere gli ostacoli per studenti, insegnanti e scuole che vogliono programmare robot educativi. Open Roberta è finanziata da Google.org (2014 - 2015) con un millione di euro.</a>";
Blockly.Msg.POPUP_AGE = "Age";
Blockly.Msg.POPUP_ATTENTION = "Attenzione";
Blockly.Msg.POPUP_BEFOREUNLOAD = "Ci sono cambiamenti che non sono stati salvati nel tuo programma o nella configurazione. Accedi al tuo account e salva il programma o la configurazione.";
Blockly.Msg.POPUP_BEFOREUNLOAD_LOGGEDIN = "Ci sono cambiamenti che non sono stati salvati nel tuo programma o nella configurazione.";
Blockly.Msg.POPUP_CANCEL = "Annulla";
Blockly.Msg.POPUP_CHANGE_PASSWORD = "cambia la password ...";
Blockly.Msg.POPUP_CONFIRM_DELETE_CONFIGURATION = "Vuoi veramente cancellare la configurazione?";
Blockly.Msg.POPUP_CONFIRM_DELETE_PROGRAM = "Vuoi veramente cancellare questo programma?";
Blockly.Msg.POPUP_CONFIRM_UPDATE_FIRMWARE = "Una nuova versione del firmware per il tuo robot è disponibile. Puoi provare a eseguire programmi con la versione vecchia, però la soluzione migliore sarebbe aggiornare il robot adesso! Devi solo cliccare >>Aggiorna subito<<.";
Blockly.Msg.POPUP_CONTINUE = "continua comunque";
Blockly.Msg.POPUP_DOWNLOAD = "Download your program to »$«";
Blockly.Msg.POPUP_DOWNLOAD_CHECK = "Okay, I've changed the download folder of my browser permanently. Don't show this popup again and download my programs directly.";
Blockly.Msg.POPUP_DOWNLOAD_SAVE = "Save";
Blockly.Msg.POPUP_DOWNLOAD_SAVE_AS = "Save link as ...";
Blockly.Msg.POPUP_DOWNLOAD_STEP_A = "Right click on your program link below and";
Blockly.Msg.POPUP_DOWNLOAD_STEP_B = "choose »Save link as ...«, then";
Blockly.Msg.POPUP_DOWNLOAD_STEP_C = "click on your connected »$« in the left column,";
Blockly.Msg.POPUP_DOWNLOAD_STEP_D = "now click on the »Save« button on the bottom right.<br><span style='font-size: 14px;'>If your program doesn't start automatically press the reset button after a while.</span>";
Blockly.Msg.POPUP_DO_UPDATE_FIRMWARE = "Aggiorna subito";
Blockly.Msg.POPUP_EMAIL = "E-mail";
Blockly.Msg.POPUP_EMAIL_SEND = "Invia adesso";
Blockly.Msg.POPUP_GET_LINK = "Here is the link to your actual program. Please don't change it, it probably won't work anymore. It's already copied to your clipboard!</br>$";
Blockly.Msg.POPUP_NAME = "Nome";
Blockly.Msg.POPUP_NEW_PASSWORD = "Nuova password";
Blockly.Msg.POPUP_OLD_PASSWORD = "Vecchia password";
Blockly.Msg.POPUP_PASSWORD = "Password";
Blockly.Msg.POPUP_PASSWORD_RECOVERY = "resetta password ...";
Blockly.Msg.POPUP_PROGRAM_TERMINATED_UNEXPECTED = "L'esecuzione del programma sul robot è terminata in modo imprevisto!";
Blockly.Msg.POPUP_REGISTER_USER = "Registrati adesso";
Blockly.Msg.POPUP_REPEAT_PASSWORD = "Ripeti la password";
Blockly.Msg.POPUP_ROBOT_BATTERY = "Tensione (V)";
Blockly.Msg.POPUP_ROBOT_NAME = "Nome";
Blockly.Msg.POPUP_ROBOT_NOT_CONNECTED = "Prima devi connetterti al tuo robot.";
Blockly.Msg.POPUP_ROBOT_STATE = "Stato";
Blockly.Msg.POPUP_ROBOT_STATE_BUSY = "occupato";
Blockly.Msg.POPUP_ROBOT_STATE_DISCONNECTED = "disconnesso";
Blockly.Msg.POPUP_ROBOT_STATE_WAIT = "aspetta";
Blockly.Msg.POPUP_ROBOT_SYSTEM = "Sistema";
Blockly.Msg.POPUP_ROBOT_WAIT = "Tempo d'attesa";
Blockly.Msg.POPUP_STARTUP_COOKIES = "Usiamo cookies per personalizzare il contenuto e per analizzare il nostro traffico.";
Blockly.Msg.POPUP_STARTUP_HELP = "Ti serve aiuto?";
Blockly.Msg.POPUP_STARTUP_HELP_TEXT = "In our detailed help, we will explain everything you need, from building instructions to frequently asked questions.";
Blockly.Msg.POPUP_STARTUP_HIDE = "Ok, non mostrare più questa finestra e ricorda la mia scelta.";
Blockly.Msg.POPUP_STARTUP_START = "Seleziona il tuo sistema!";
Blockly.Msg.POPUP_STARTUP_TOUR_TEXT = "Would you like to get started, but do not know exactly how? We will show you the first steps in an interactive tutorial.";
Blockly.Msg.POPUP_TOUR = "fai un tour";
Blockly.Msg.POPUP_USERNAME = "Nome utente";
Blockly.Msg.POPUP_USERNAME_LOGOFF = "Non sei autenticato/connesso/registrato.";
Blockly.Msg.POPUP_VALUE = "Valore";
Blockly.Msg.PROCEDURES_ALLOW_STATEMENTS = "consenti dichiarazioni";
Blockly.Msg.PROCEDURES_BEFORE_PARAMS = "conː";
Blockly.Msg.PROCEDURES_CALLNORETURN_HELPURL = "https://it.wikipedia.org/wiki/Funzione_(informatica)";
Blockly.Msg.PROCEDURES_CALLNORETURN_TOOLTIP = "Esegue la funzione '%1' definita dall'utente.";
Blockly.Msg.PROCEDURES_CALLRETURN_HELPURL = "https://it.wikipedia.org/wiki/Funzione_(informatica)";
Blockly.Msg.PROCEDURES_CALLRETURN_TOOLTIP = "Esegue la funzione '%1' definita dall'utente e ne ottiene il risultato.";
Blockly.Msg.PROCEDURES_CALL_BEFORE_PARAMS = "conː";
Blockly.Msg.PROCEDURES_CREATE_DO = "Crea '%1'";
Blockly.Msg.PROCEDURES_DEFNORETURN_COMMENT = "Descrivi questa funzione...";
Blockly.Msg.PROCEDURES_DEFNORETURN_DO = "";
Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL = "https://en.wikipedia.org/wiki/Procedure_%28computer_science%29";
Blockly.Msg.PROCEDURES_DEFNORETURN_PROCEDURE = "perFareQualcosa";
Blockly.Msg.PROCEDURES_DEFNORETURN_TITLE = "per eseguire";
Blockly.Msg.PROCEDURES_DEFNORETURN_TOOLTIP = "Crea una funzione senza risultato.";
Blockly.Msg.PROCEDURES_DEFRETURN_COMMENT = "Descrivi questa funzione...";
Blockly.Msg.PROCEDURES_DEFRETURN_DO = "";
Blockly.Msg.PROCEDURES_DEFRETURN_HELPURL = "https://en.wikipedia.org/wiki/Procedure_%28computer_science%29";
Blockly.Msg.PROCEDURES_DEFRETURN_PROCEDURE = "perCalcolareQualcosa";
Blockly.Msg.PROCEDURES_DEFRETURN_RETURN = "ritorna";
Blockly.Msg.PROCEDURES_DEFRETURN_TITLE = "per calcolare";
Blockly.Msg.PROCEDURES_DEFRETURN_TOOLTIP = "Crea una funzione che torna un risultato.";
Blockly.Msg.PROCEDURES_DEF_DUPLICATE_WARNING = "Attenzioneː Questa funzione ha parametri duplicati.";
Blockly.Msg.PROCEDURES_HIGHLIGHT_DEF = "Evidenzia la definizione di funzione";
Blockly.Msg.PROCEDURES_IFRETURN_HELPURL = "http://c2.com/cgi/wiki?GuardClause";
Blockly.Msg.PROCEDURES_IFRETURN_TOOLTIP = "Se il primo valore è vero allora la funzione che racchiude questo blocco ritorna come risultato il secondo valore.";
Blockly.Msg.PROCEDURES_IFRETURN_WARNING = "Attenzioneː Questo blocco può essere usato solo all'interno di una definizione di funzione.";
Blockly.Msg.PROCEDURES_MUTATORARG_TITLE = "nome inputː";
Blockly.Msg.PROCEDURES_MUTATORARG_TOOLTIP = "Aggiungi un input alla funzione.";
Blockly.Msg.PROCEDURES_MUTATORCONTAINER_TITLE = "input";
Blockly.Msg.PROCEDURES_MUTATORCONTAINER_TOOLTIP = "Aggiungi, rimuovi o riordina input alla funzione.";
Blockly.Msg.PROCEDURES_TITLE = "procedura";
Blockly.Msg.PROCEDURES_VARIABLES_ERROR = "Errore: Questo blocco può solo essere usato all'interno di '";
Blockly.Msg.PROCEDURES_VARIABLES_LOOP_ERROR = "Errore: Questo blocco può solo essere usato entro un circuito che dichiara ";
Blockly.Msg.PROGLIST_DELETE_ALL_TOOLTIP = "Click here to delete all selected programs.";
Blockly.Msg.PROGLIST_DELETE_SHARE_WITH_GALLERY = "Your program is allready shared with the gallery. If you change it, it will be automatically updated. Do you want to remove your program from the gallery?";
Blockly.Msg.PROGLIST_DELETE_TOOLTIP = "Click here to delete your program.";
Blockly.Msg.PROGLIST_LOAD_TOOLTIP = "Click here to load your robot configuration in the configuration environment.";
Blockly.Msg.PROGLIST_SHARE_TOOLTIP = "Click here to share your program with a friend.";
Blockly.Msg.PROGLIST_SHARE_WITH_GALLERY = "Do you really want to share your program with everybody?";
Blockly.Msg.PULSE_HIGH = "pulse time HIGH";
Blockly.Msg.PULSE_LOW = "pulse time LOW";
Blockly.Msg.REDO = "Ripeti";
Blockly.Msg.REMOVE_COMMENT = "Rimuovi commento";
Blockly.Msg.RENAME_VARIABLE = "Rinomina variabile...";
Blockly.Msg.RENAME_VARIABLE_TITLE = "Rinomina tutte le variabili '%1' in:";
Blockly.Msg.RETURN = "ritorna";
Blockly.Msg.ROLL = "roll";
Blockly.Msg.SEND_DATA = "dati da inviare";
Blockly.Msg.SENSOR_AMBIENTLIGHT = "sensore di luce ambiente";
Blockly.Msg.SENSOR_BATTERY = "tensione in V";
Blockly.Msg.SENSOR_CODE = "code pad";
Blockly.Msg.SENSOR_COLOUR = "sensore di colore/luce";
Blockly.Msg.SENSOR_COMPASS = "sensore bussola";
Blockly.Msg.SENSOR_ENCODER = "encoder del motore";
Blockly.Msg.SENSOR_FLAME = "flame sensor";
Blockly.Msg.SENSOR_GESTURE = "gesture";
Blockly.Msg.SENSOR_GESTURE_ACTIVE = "active?";
Blockly.Msg.SENSOR_GESTURE_DOWN = "upside down";
Blockly.Msg.SENSOR_GESTURE_FACE_DOWN = "at the front side";
Blockly.Msg.SENSOR_GESTURE_FACE_UP = "at the back";
Blockly.Msg.SENSOR_GESTURE_FREEFALL = "freely falling";
Blockly.Msg.SENSOR_GESTURE_SHAKE = "shaking";
Blockly.Msg.SENSOR_GESTURE_UP = "upright";
Blockly.Msg.SENSOR_GET = "ottieni";
Blockly.Msg.SENSOR_GET_SAMPLE = "ottieni il valore del";
Blockly.Msg.SENSOR_GROVE = "Grove";
Blockly.Msg.SENSOR_GYRO = "giroscopio";
Blockly.Msg.SENSOR_INFRARED = "sensore infrarosso";
Blockly.Msg.SENSOR_IS_PIN = "is";
Blockly.Msg.SENSOR_IS_PRESSED = "premuto?";
Blockly.Msg.SENSOR_IS_TOUCHED = "toccato?";
Blockly.Msg.SENSOR_JOYSTICK = "joystick";
Blockly.Msg.SENSOR_KEY = "tasto";
Blockly.Msg.SENSOR_KEY_ANY = "qualsiasi";
Blockly.Msg.SENSOR_KEY_DOWN = "giù";
Blockly.Msg.SENSOR_KEY_ENTER = "invio/enter";
Blockly.Msg.SENSOR_KEY_ESCAPE = "escape";
Blockly.Msg.SENSOR_KEY_LEFT = "a sinistra";
Blockly.Msg.SENSOR_KEY_RIGHT = "a destra";
Blockly.Msg.SENSOR_KEY_UP = "su";
Blockly.Msg.SENSOR_LIGHT = "sensore di luce";
Blockly.Msg.SENSOR_MIC = "microfono";
Blockly.Msg.SENSOR_MS_TIMER = "in ms (millisecondi)";
Blockly.Msg.SENSOR_PIN = "pin";
Blockly.Msg.SENSOR_PIRMOTION = "PIR Motion sensor";
Blockly.Msg.SENSOR_PRESSED = "premuto";
Blockly.Msg.SENSOR_RESET = "azzera";
Blockly.Msg.SENSOR_RESET_II = "";
Blockly.Msg.SENSOR_SONAR = "sonar";
Blockly.Msg.SENSOR_SOUND = "sensore di suono";
Blockly.Msg.SENSOR_TEMPERATURE = "sensore di temperatura";
Blockly.Msg.SENSOR_TIME = "tempo";
Blockly.Msg.SENSOR_TIMER = "cronometro";
Blockly.Msg.SENSOR_TOUCH = "pulsante";
Blockly.Msg.SENSOR_ULTRASONIC = "sensore ad ultrasuoni";
Blockly.Msg.SENSOR_VALUE = "valore";
Blockly.Msg.SERVER_NOT_AVAILABLE = "Lo Open Roberta Lab non è disponibile al momento. Per favore controlla la tua connessione a internet e prova a ricaricare questa pagina. Se il problema persiste, per favore contattaci tramite e-mail: sterbini@di.uniroma1.it";
Blockly.Msg.SET = "imposta";
Blockly.Msg.SIM_BLOCK_NOT_SUPPORTED = "Sfortunatamente non puoi usare questo blocco nella simulazione!";
Blockly.Msg.SIM_CONFIGURATION_WARNING_SENSOR_MISSING = "Questo sensore non c'è nella configurazione! Il comportamento del programma sul robot vero potrebbe essere diverso da quello previsto!";
Blockly.Msg.SIM_CONFIGURATION_WARNING_WRONG_INFRARED_SENSOR_PORT = "Il sensore infrarosso non è connesso nella configurazione! Il comportamento del programma sul robot vero potrebbe essere diverso da quello previsto!";
Blockly.Msg.SIM_CONFIGURATION_WARNING_WRONG_SENSOR_PORT = "In questo blocco hai usato la porta del sensore sbagliata! Il comportamento del programma sul robot vero potrebbe essere diverso da quello previsto!";
Blockly.Msg.SLEEP = "sleep";
Blockly.Msg.SOUND_GETSAMPLE_TOOLTIP = "Ottiene il valore del sensore di suono.";
Blockly.Msg.SOUND_TOOLTIP = "Sensore di suono.";
Blockly.Msg.START = "start/avvia/inizio";
Blockly.Msg.START_ACTIVITY = "attività";
Blockly.Msg.START_ACTIVITY_TOOLTIP = "Avvia una attività aggiuntiva.";
Blockly.Msg.START_PROGRAM = "avvia il programma";
Blockly.Msg.START_PROGRAM_DEBUG = "mostra i dati dei sensori";
Blockly.Msg.START_TOOLTIP = "Il punto iniziale del programma principale.";
Blockly.Msg.STRENGTH = "strength";
Blockly.Msg.TAB_CONFIGURATION = "Configurazione del robot";
Blockly.Msg.TAB_PROGRAM = "Programma";
Blockly.Msg.TEMPERATURE_GETSAMPLE_TOOLTIP = "Gets the current reading from the temperatur sensor.";
Blockly.Msg.TEMPERATURE_TOOLTIP = "Represents a temperature sensor.";
Blockly.Msg.TEXT_APPEND_APPENDTEXT = "aggiungi il testo";
Blockly.Msg.TEXT_APPEND_HELPURL = "https://github.com/google/blockly/wiki/Text#text-modification";
Blockly.Msg.TEXT_APPEND_TO = "a";
Blockly.Msg.TEXT_APPEND_TOOLTIP = "Aggiunge del testo alla variabile '%1'.";
Blockly.Msg.TEXT_CHANGECASE_HELPURL = "https://github.com/google/blockly/wiki/Text#adjusting-text-case";
Blockly.Msg.TEXT_CHANGECASE_OPERATOR_LOWERCASE = "in minuscolo";
Blockly.Msg.TEXT_CHANGECASE_OPERATOR_TITLECASE = "con Iniziali Maiuscole";
Blockly.Msg.TEXT_CHANGECASE_OPERATOR_UPPERCASE = "in MAIUSCOLO";
Blockly.Msg.TEXT_CHANGECASE_TOOLTIP = "Restituisce una copia del testo in un diverso formato maiuscole/minuscole.";
Blockly.Msg.TEXT_CHARAT_FIRST = "prendi la prima lettera";
Blockly.Msg.TEXT_CHARAT_FROM_END = "prendi la lettera alla posizione (contando dalla fine)";
Blockly.Msg.TEXT_CHARAT_FROM_START = "prendi la lettera alla posizione";
Blockly.Msg.TEXT_CHARAT_HELPURL = "https://github.com/google/blockly/wiki/Text#extracting-text";
Blockly.Msg.TEXT_CHARAT_INPUT_INTEXT = "nel testo";
Blockly.Msg.TEXT_CHARAT_LAST = "prendi l'ultima lettera";
Blockly.Msg.TEXT_CHARAT_RANDOM = "prendi lettera casuale";
Blockly.Msg.TEXT_CHARAT_TAIL = "";
Blockly.Msg.TEXT_CHARAT_TOOLTIP = "Restituisce la lettera nella posizione indicata.";
Blockly.Msg.TEXT_CREATE_JOIN_ITEM_TOOLTIP = "Aggiungi un elemento al testo.";
Blockly.Msg.TEXT_CREATE_JOIN_TITLE_JOIN = "unisci";
Blockly.Msg.TEXT_CREATE_JOIN_TOOLTIP = "Aggiungi, rimuovi o riordina le sezioni per riconfigurare questo blocco testo.";
Blockly.Msg.TEXT_GET_SUBSTRING_END_FROM_END = "alla lettera alla posizione (contando dalla fine)";
Blockly.Msg.TEXT_GET_SUBSTRING_END_FROM_START = "alla lettera alla posizione";
Blockly.Msg.TEXT_GET_SUBSTRING_END_LAST = "all'ultima lettera";
Blockly.Msg.TEXT_GET_SUBSTRING_HELPURL = "https://github.com/google/blockly/wiki/Text#extracting-a-region-of-text";
Blockly.Msg.TEXT_GET_SUBSTRING_INPUT_IN_TEXT = "nel testo";
Blockly.Msg.TEXT_GET_SUBSTRING_START_FIRST = "prendi sotto-stringa dalla prima lettera";
Blockly.Msg.TEXT_GET_SUBSTRING_START_FROM_END = "prendi sotto-stringa dalla lettera alla posizione (contando dalla fine)";
Blockly.Msg.TEXT_GET_SUBSTRING_START_FROM_START = "prendi sotto-stringa dalla lettera alla posizione";
Blockly.Msg.TEXT_GET_SUBSTRING_TAIL = "";
Blockly.Msg.TEXT_GET_SUBSTRING_TOOLTIP = "Restituisce la porzione di testo indicata.";
Blockly.Msg.TEXT_INDEXOF_HELPURL = "https://github.com/google/blockly/wiki/Text#finding-text";
Blockly.Msg.TEXT_INDEXOF_INPUT_INTEXT = "nel testo";
Blockly.Msg.TEXT_INDEXOF_OPERATOR_FIRST = "trova la prima occorrenza del testo";
Blockly.Msg.TEXT_INDEXOF_OPERATOR_LAST = "trova l'ultima occorrenza del testo";
Blockly.Msg.TEXT_INDEXOF_TAIL = "";
Blockly.Msg.TEXT_INDEXOF_TOOLTIP = "Restituisce l'indice della prima occorrenza del primo testo all'interno del secondo testo. Restituisce 0 se il testo non viene trovato.";
Blockly.Msg.TEXT_ISEMPTY_HELPURL = "https://github.com/google/blockly/wiki/Text#checking-for-empty-text";
Blockly.Msg.TEXT_ISEMPTY_TITLE = "%1 è vuoto";
Blockly.Msg.TEXT_ISEMPTY_TOOLTIP = "Restituisce vero se il testo fornito è vuoto.";
Blockly.Msg.TEXT_JOIN_HELPURL = "https://github.com/google/blockly/wiki/Text#text-creation";
Blockly.Msg.TEXT_JOIN_TITLE_CREATEWITH = "crea testo concatenando";
Blockly.Msg.TEXT_JOIN_TOOLTIP = "Crea un blocco di testo unendo un certo numero di elementi.";
Blockly.Msg.TEXT_LENGTH_HELPURL = "https://github.com/google/blockly/wiki/Text#text-modification";
Blockly.Msg.TEXT_LENGTH_TITLE = "lunghezza di %1";
Blockly.Msg.TEXT_LENGTH_TOOLTIP = "Restituisce il numero di lettere (inclusi gli spazi) nel testo fornito.";
Blockly.Msg.TEXT_PRINT_HELPURL = "https://github.com/google/blockly/wiki/Text#printing-text";
Blockly.Msg.TEXT_PRINT_TITLE = "scrivi %1";
Blockly.Msg.TEXT_PRINT_TOOLTIP = "Scrive il testo, numero o altro valore indicato.";
Blockly.Msg.TEXT_PROMPT_HELPURL = "https://github.com/google/blockly/wiki/Text#getting-input-from-the-user";
Blockly.Msg.TEXT_PROMPT_TOOLTIP_NUMBER = "Richiedi un numero all'utente.";
Blockly.Msg.TEXT_PROMPT_TOOLTIP_TEXT = "Richiede del testo da parte dell'utente.";
Blockly.Msg.TEXT_PROMPT_TYPE_NUMBER = "richiedi numero con messaggio";
Blockly.Msg.TEXT_PROMPT_TYPE_TEXT = "richiedi testo con messaggio";
Blockly.Msg.TEXT_TEXT_HELPURL = "https://it.wikipedia.org/wiki/Stringa_(informatica)";
Blockly.Msg.TEXT_TEXT_TOOLTIP = "Una lettera, una parola o una linea di testo.";
Blockly.Msg.TEXT_TRIM_HELPURL = "https://github.com/google/blockly/wiki/Text#trimming-removing-spaces";
Blockly.Msg.TEXT_TRIM_OPERATOR_BOTH = "rimuovi spazi da entrambi gli estremi";
Blockly.Msg.TEXT_TRIM_OPERATOR_LEFT = "rimuovi spazi a sinistra";
Blockly.Msg.TEXT_TRIM_OPERATOR_RIGHT = "rimuovi spazi a destra";
Blockly.Msg.TEXT_TRIM_TOOLTIP = "Restituisce una copia del testo con gli spazi rimossi ad uno o entrambe le estremità.";
Blockly.Msg.TIMEOUT = "timeout";
Blockly.Msg.TIMER_GETSAMPLE_TOOLTIP = "Ottiene il valore corrente del cronometro.";
Blockly.Msg.TIMER_RESET_TOOLTIP = "Azzera il cronometro.";
Blockly.Msg.TODAY = "Oggi";
Blockly.Msg.TOOLBOX_ACTION = "Azioni";
Blockly.Msg.TOOLBOX_ANIMATION = "Animation";
Blockly.Msg.TOOLBOX_COLOUR = "Colori";
Blockly.Msg.TOOLBOX_COMMUNICATION = "Messaggi";
Blockly.Msg.TOOLBOX_CONTROL = "Controllo";
Blockly.Msg.TOOLBOX_DECISION = "Decisioni";
Blockly.Msg.TOOLBOX_DISPLAY = "Schermo";
Blockly.Msg.TOOLBOX_DRIVE = "Guida";
Blockly.Msg.TOOLBOX_IMAGE = "Images";
Blockly.Msg.TOOLBOX_LIGHT = "Luci";
Blockly.Msg.TOOLBOX_LIST = "Liste";
Blockly.Msg.TOOLBOX_LOGIC = "Logica";
Blockly.Msg.TOOLBOX_LOOP = "Cicli";
Blockly.Msg.TOOLBOX_MATH = "Matematica";
Blockly.Msg.TOOLBOX_MOVE = "Motori";
Blockly.Msg.TOOLBOX_PIN = "Pin";
Blockly.Msg.TOOLBOX_PROCEDURE = "Funzioni";
Blockly.Msg.TOOLBOX_SENSOR = "Sensori";
Blockly.Msg.TOOLBOX_SOUND = "Suoni";
Blockly.Msg.TOOLBOX_TEXT = "Testo";
Blockly.Msg.TOOLBOX_VARIABLE = "Variabili";
Blockly.Msg.TOOLBOX_VISION = "Vision";
Blockly.Msg.TOOLBOX_WAIT = "Aspetta";
Blockly.Msg.TOOLBOX_WALK = "Walk";
Blockly.Msg.TOUCH_ISPRESSED_TOOLTIP = "Il sensore pulsante è premuto?";
Blockly.Msg.TOUCH_TOOLTIP = "Il sensore pulsante è premuto?";
Blockly.Msg.TOUR1_DESCRIPTION00 = "next";
Blockly.Msg.TOUR1_DESCRIPTION01 = "Welcome,<br>click on  »next« and start a tour through the Open Roberta Lab.<br>I'll show you how you can program your robot with NEPO.<br>You can end this tour all the time with a click on »X« in the right upper corner.";
Blockly.Msg.TOUR1_DESCRIPTION02 = "The menu ;-)";
Blockly.Msg.TOUR1_DESCRIPTION03 = "Here you can find everything you need to program the robot!<br><span class='typcn typcn-document-text'></span> Save and load programs<br><span class='typcn typcn-Roberta'></span> Connect your robot to the Open Roberta Lab<br><span class='typcn typcn-lightbulb'></span> Help for all kinds of problems<br><span class='typcn typcn-user'></span>Everything about user accounts, e.g. login or logoff";
Blockly.Msg.TOUR1_DESCRIPTION04 = "Click here and switch to the robot configuration";
Blockly.Msg.TOUR1_DESCRIPTION05 = "You can adapt the robot configuration later,<br>if you've constructed another robot.<br>If you don't have a robot and want to test your program in the simulation <br>this configuration is applied.";
Blockly.Msg.TOUR1_DESCRIPTION06 = "Click here and switch back to the programming area!";
Blockly.Msg.TOUR1_DESCRIPTION07 = "The toolbox.<br>Here you can find all programming blocks.";
Blockly.Msg.TOUR1_DESCRIPTION08 = "Each program starts with the program-start-block«.<br>Further programming blocks, which the robot should execute, should be attached to this block.<br>Just drag'n drop the desired block right under the start block.";
Blockly.Msg.TOUR1_DESCRIPTION09 = "Here you find some shortcuts!<br>From left to right:<br><span class='typcn typcn-media-play'></span> Click here to execute the program on the real robot<br><span class='typcn typcn-media-play-outline'></span> Click here to start the robot in the simulation<br><span class='typcn typcn-cloud-storage'></span> Save your program, but before that you need to sign in<br><span class='typcn typcn-zoom'></span> Click here and zoom in on the blocks, if you want to<br><span class='typcn typcn-archive'></span> The trashcan! Just move blocks into the trashcan and they will be deleted.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Attention: You cannot recover blocks once they are deleted!<br>";
Blockly.Msg.TOUR1_DESCRIPTION10 = "Click on »Action«";
Blockly.Msg.TOUR1_DESCRIPTION12 = "Now drag'n drop the new block just below the start-program-block so that they are connected";
Blockly.Msg.TOUR1_DESCRIPTION13 = "Now open the simulation ...";
Blockly.Msg.TOUR1_DESCRIPTION13a = "and test your program in the simulation!";
Blockly.Msg.TOUR1_DESCRIPTION15 = "Perfect,<br>your robot is driving 20 cm forward,<br>this is what you have programmed!";
Blockly.Msg.TOUR1_DESCRIPTION16 = "Bye and have fun trying out more!";
Blockly.Msg.ULTRASONIC_GETSAMPLE_TOOLTIP = "Ottiene il valore (distanza in cm) dal sensore ad ultrasuoni.";
Blockly.Msg.ULTRASONIC_TOOLTIP = "Rappresenta il sensore ad ultrasuoni del brick EV3.";
Blockly.Msg.UNDO = "Annulla";
Blockly.Msg.VALIDATION_CONTAINS_SPECIAL_CHARACTERS = "Valid special characters: »=+!?.,%#+&^@_-«";
Blockly.Msg.VALIDATION_FIELD_REQUIRED = "Questo campo non può essere vuoto!";
Blockly.Msg.VALIDATION_MAX_LENGTH = "The max. lenght is 25 characters!";
Blockly.Msg.VALIDATION_PASSWORD_MIN_LENGTH = "La lunghezza minima della password è 6!";
Blockly.Msg.VALIDATION_SECOND_PASSWORD_EQUAL = "Le due password devono essere uguali!";
Blockly.Msg.VALIDATION_TOKEN_LENGTH = "Il Token dev'essere di 8 caratteri!";
Blockly.Msg.VALIDATION_VALID_EMAIL_ADDRESS = "Per favore inserisci un indirizzo e-mail valido!";
Blockly.Msg.VALUE_TO = "value to";
Blockly.Msg.VARIABLES_DEFAULT_NAME = "nome";
Blockly.Msg.VARIABLES_GET_CREATE_SET = "Crea 'imposta %1'";
Blockly.Msg.VARIABLES_GET_HELPURL = "https://github.com/google/blockly/wiki/Variables#get";
Blockly.Msg.VARIABLES_GET_TOOLTIP = "Restituisce il valore di una variabile.";
Blockly.Msg.VARIABLES_GLOBAL_DECLARE_TOOLTIP = "Definisci una variabile globale.";
Blockly.Msg.VARIABLES_LOCAL_DECLARE_TOOLTIP = "Definisci una variabile locale.";
Blockly.Msg.VARIABLES_SET = "imposta %1 al valore %2";
Blockly.Msg.VARIABLES_SET_CREATE_GET = "Crea 'prendi %1'";
Blockly.Msg.VARIABLES_SET_HELPURL = "https://github.com/google/blockly/wiki/Variables#set";
Blockly.Msg.VARIABLES_SET_TOOLTIP = "Imposta il valore di questa variabile ad essere pari all'input.";
Blockly.Msg.VARIABLES_TITLE = "variabile";
Blockly.Msg.VARIABLES_TYPE_ARRAY_BOOLEAN = "Lista di valori booleani";
Blockly.Msg.VARIABLES_TYPE_ARRAY_COLOUR = "Lista di colori";
Blockly.Msg.VARIABLES_TYPE_ARRAY_CONNECTION = "Lista di connessioni";
Blockly.Msg.VARIABLES_TYPE_ARRAY_IMAGE = "List Image";
Blockly.Msg.VARIABLES_TYPE_ARRAY_NUMBER = "Lista di numeri";
Blockly.Msg.VARIABLES_TYPE_ARRAY_STRING = "Lista di testi";
Blockly.Msg.VARIABLES_TYPE_BOOLEAN = "Booleano (vero/falso)";
Blockly.Msg.VARIABLES_TYPE_COLOUR = "Colore";
Blockly.Msg.VARIABLES_TYPE_CONNECTION = "Connessione";
Blockly.Msg.VARIABLES_TYPE_IMAGE = "Image";
Blockly.Msg.VARIABLES_TYPE_NUMBER = "Numero";
Blockly.Msg.VARIABLES_TYPE_STRING = "Testo";
Blockly.Msg.VARIABLE_USED_BEFORE_DECLARATION = "The variable is used before declaration.";
Blockly.Msg.WAIT = "aspetta (ms)";
Blockly.Msg.WAIT_FOR_TOOLTIP = "Attende i valori dei sensori.";
Blockly.Msg.WAIT_OR = "oppure aspetta finché";
Blockly.Msg.WAIT_TIME_TOOLTIP = "Attendi un certo numero di millisecondi.";
Blockly.Msg.WAIT_TOOLTIP = "Aspetta che una condizione diventi vera.";
Blockly.Msg.WAIT_UNTIL = "aspetta finché";
Blockly.Msg.WRITE_TO_PIN_TOOLTIP = "Sends the value to chosen pin.";
Blockly.Msg.X = "x";
Blockly.Msg.Y = "y";
Blockly.Msg.YAW = "yaw";
Blockly.Msg.YES = "sì";
Blockly.Msg.YOUNGER_THEN_14 = "I am younger then 14!";
Blockly.Msg.AS_PROCEDURES_DEFRETURN_COMMENT = Blockly.Msg.PROCEDURES_DEFNORETURN_COMMENT;
Blockly.Msg.CONTROLS_IF_IF_TITLE_IF = Blockly.Msg.CONTROLS_IF_MSG_IF;
Blockly.Msg.CONTROLS_WHILEUNTIL_INPUT_DO = Blockly.Msg.CONTROLS_REPEAT_INPUT_DO;
Blockly.Msg.CONTROLS_IF_MSG_THEN = Blockly.Msg.CONTROLS_REPEAT_INPUT_DO;
Blockly.Msg.CONTROLS_FOR_INPUT_DO = Blockly.Msg.CONTROLS_REPEAT_INPUT_DO;
Blockly.Msg.CONTROLS_IF_ELSE_TITLE_ELSE = Blockly.Msg.CONTROLS_IF_MSG_ELSE;
Blockly.Msg.AS_PROCEDURES_DEFRETURN_DO = Blockly.Msg.PROCEDURES_DEFNORETURN_DO;
Blockly.Msg.LISTS_GET_SUBLIST_INPUT_IN_LIST = Blockly.Msg.LISTS_INLIST;
Blockly.Msg.LISTS_GET_INDEX_INPUT_IN_LIST = Blockly.Msg.LISTS_INLIST;
Blockly.Msg.CONTROLS_IF_ELSEIF_TITLE_ELSEIF = Blockly.Msg.CONTROLS_IF_MSG_ELSEIF;
Blockly.Msg.LISTS_GET_INDEX_HELPURL = Blockly.Msg.LISTS_INDEX_OF_HELPURL;
Blockly.Msg.AS_PROCEDURES_DEFRETURN_PROCEDURE = Blockly.Msg.PROCEDURES_DEFNORETURN_PROCEDURE;
Blockly.Msg.TEXT_APPEND_VARIABLE = Blockly.Msg.VARIABLES_DEFAULT_NAME;
Blockly.Msg.CONTROLS_FOREACH_INPUT_DO = Blockly.Msg.CONTROLS_REPEAT_INPUT_DO;
Blockly.Msg.LISTS_CREATE_WITH_ITEM_TITLE = Blockly.Msg.VARIABLES_DEFAULT_NAME;
Blockly.Msg.MATH_CHANGE_TITLE_ITEM = Blockly.Msg.VARIABLES_DEFAULT_NAME;
Blockly.Msg.TEXT_CREATE_JOIN_ITEM_TITLE_ITEM = Blockly.Msg.VARIABLES_DEFAULT_NAME;
Blockly.Msg.LISTS_INDEX_OF_INPUT_IN_LIST = Blockly.Msg.LISTS_INLIST;
Blockly.Msg.LISTS_SET_INDEX_INPUT_IN_LIST = Blockly.Msg.LISTS_INLIST;
Blockly.Msg.AS_PROCEDURES_DEFRETURN_TITLE = Blockly.Msg.PROCEDURES_DEFNORETURN_TITLE;